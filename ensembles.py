...
comp_data = {'C1':C_tab['C1'], 'C2':C_tab['C2'],
             'M1':M_tab['M1'], 'M2':M_tab['M2']} 
...
for i in range(len(ensembles)):
    ...
    for i in range(5):
        for j in range(5):
            k, l = i, j
            if mask[k,l]:
                ax1[i,j].scatter(momenta,[all_data[ens]['Z'][m][k,l]
                                for m in momenta], label=ens,
                                c=color, alpha=0.75)
                ax1[i,j].scatter([3],[pt2[k,l]], facecolor=color,
                                 edgecolor='black',alpha=0.75, marker='*')
                ax1[i,j].set_ylabel('$Z_{'+str(k+1)+','+str(l+1)+'}/Z_V^2$')
                ax1[i,j].set_xlabel('$q/GeV$')
            else:
                ax1[i,j].axis('off')

    ax2.scatter(momenta,[(all_data[ens]['proj'][m][0,0]/fq_gamma_F[0,0]).real
                         for m in momenta], label='RM',c=color,alpha=0.75)
...
fig2.suptitle('$\Lambda_{VV+AA}}$ in ($\gamma_{\mu}$,$\gamma_{\mu}$) scheme\no:RM,*:1411.7017')
plt.show()
