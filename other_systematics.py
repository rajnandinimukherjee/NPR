...
if run:
    ...
    record_vals = {
        "mask_pre_inv": {"kwargs": {"resid_mask": True, "mask": fq_mask.copy()}},
        "mask_post_inv": {
            "kwargs": {"resid_mask": False, "mask": fq_mask.copy(), "run_extrap": True}
        },
        "no_mask": {
            "kwargs": {
    ...
    NPR_to_SUSY_stat = stat(
        val=NPR_to_SUSY, err=np.zeros(shape=NPR_to_SUSY.shape), btsp="fill"
    )
    ...
    del b
    r = bag_fits(
        bag_ensembles, obj="ratio", mask=fq_mask.copy(), resid_mask=False, scheme=scheme
    )
    ratios_1 = [
        r.fit_operator(
            mu1,
    ...
    for fit in list(record_vals.keys())[:-1]:
        print(fit)
        b = bag_fits(
            bag_ensembles, obj="bag", scheme=scheme, **record_vals[fit]["kwargs"]
        )
        bags_1 = [
            b.fit_operator(
                mu1,
        ...
        sig_b = s_b.calc_running(
            mu1, mu2, rotate=NPR_to_SUSY, chiral_extrap=True, chi_sq_rescale=True
        )
        ...
        del b
        r = bag_fits(
            bag_ensembles, obj="ratio", scheme=scheme, **record_vals[fit]["kwargs"]
        )
        ratios_1 = [
            r.fit_operator(
                mu1,
