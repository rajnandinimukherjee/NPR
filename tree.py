...
prop_in = (
    (prop_in_data["re"] + prop_in_data["im"] * 1j).swapaxes(1, 2).reshape((12, 12))
)
...
prop_out = (
    (prop_out_data["re"] + prop_out_data["im"] * 1j).swapaxes(1, 2).reshape((12, 12))
)
...
tree_bl_data = [tree_bl_file[f"{blstr}_{i}"]["corr"][0, 0, :] for i in range(N_bl)]
tree_bl = [
    (tree_bl_data[i]["re"] + tree_bl_data[i]["im"] * 1j)
    .swapaxes(1, 2)
    .reshape((12, 12))
    for i in range(N_bl)
]
...
p_in = np.array(
    [
        int(x)
        for x in tree_bl_file[f"{blstr}_0"]["info"].attrs["pIn"][0].decode().rsplit(" ")
    ]
)
p_out = np.array(
    [
        int(x)
        for x in tree_bl_file[f"{blstr}_0"]["info"]
        .attrs["pOut"][0]
        .decode()
        .rsplit(" ")
    ]
...
pslash = np.sum([p_in[i] * Gamma[dirs[i]] for i in range(len(dirs))], axis=0)
p_sq = np.linalg.norm(p_in) ** 2
Z_q = np.trace(-1j * in_ @ pslash).real / (12 * p_sq)

org_gammas = [
    tree_bl_file[f"{blstr}_{i}"]["info"].attrs["gamma"][0].decode() for i in range(N_bl)
]
gammas = [
    [x for x in list(org_gammas[i]) if x in list(gamma.keys())] for i in range(N_bl)
]

amputated = np.array([out_ @ tree_bl[b] @ in_ for b in range(N_bl)])
bl_operators = {
    "S": [amputated[gammas.index(["I"])]],
    "P": [amputated[gammas.index(["5"])]],
    "V": [amputated[gammas.index([i])] for i in dirs],
    "A": [amputated[gammas.index([i, "5"])] for i in dirs],
    "T": sum(
        [
            [amputated[gammas.index([dirs[i], dirs[j]])] for j in range(i + 1, 4)]
...
projected = {
    c: np.trace(
        np.sum(
            [
                bl_gamma_proj[c][i] @ bl_operators[c][i]
                for i in range(len(bl_operators[c]))
            ],
            axis=0,
        )
    )
...
Z_V = Z_q / (
    np.trace(
        np.sum([q[i] * bl_operators["V"][i] for i in range(len(dirs))], axis=0) @ qslash
    ).real
    / (12 * q_sq)
)
...
A1 = np.trace(
    np.sum([q[i] * bl_operators["A"][i] for i in range(len(dirs))], axis=0) @ Gamma["5"]
)
A2 = np.trace(
    np.sum([q[i] * bl_operators["A"][i] for i in range(len(dirs))], axis=0)
    @ Gamma["5"]
    @ qslash
)
...
def fq_SMOM():
    ...
    tree_fq_data = [tree_fq_file[f"{fqstr}_{i}"]["corr"][0, 0, :] for i in range(32)]
    tree_fq = [
        (tree_fq_data[i]["re"] + tree_fq_data[i]["im"] * 1j)
        .swapaxes(1, 2)
        .swapaxes(5, 6)
        .reshape((12, 12, 12, 12))
        for i in range(32)
    ...
    p_in = np.array(
        [
            int(x)
            for x in tree_fq_file[f"{fqstr}_0"]["info"]
            .attrs["pIn"][0]
            .decode()
            .rsplit(" ")
        ]
    ...
    org_gammas = [
        [
            tree_fq_file[f"{fqstr}_{i}"]["info"].attrs["gammaA"][0].decode(),
    ...
    gammas = [
        [
            [x for x in list(org_gammas[i][0]) if x in list(gamma.keys())],
    ...
    doublets = {
        "SS": tree_fq[gammas.index([["I"], ["I"]])],
        "PP": tree_fq[gammas.index([["5"], ["5"]])],
        "VV": np.sum(
            np.array([tree_fq[gammas.index([[i], [i]])] for i in dirs]), axis=0
        ),
        "AA": np.sum(
            np.array([tree_fq[gammas.index([[i, "5"], [i, "5"]])] for i in dirs]),
            axis=0,
        ),
        "TT": np.sum(
            np.array(
                sum(
                    [
                        [
                            tree_fq[
                                gammas.index([[dirs[i], dirs[j]], [dirs[i], dirs[j]]])
                            ]
    ...
    for k in operator.keys():
        ...
        amputated[k] = np.einsum("ea,bf,gc,dh,abcd->efgh", out_, in_, out_, in_, op_avg)

    projector = fq_gamma_projector
    projected = np.array(
        [
            [np.einsum("abcd,badc", projector[k1], amputated[k2]) for k2 in operators]
            for k1 in operators
        ]
    )
