...
for idx, ens in enumerate(['C1', 'M1', 'F1S']):
    ...
    axes[0, idx].hlines(eta_C_star, xmin, xmax, lw=2.5,
                        color=color_list[3+eta_C_star_idx])
    ...
    axes[1, idx].vlines(am_star.val, ymin, ymax, linestyle='dashed', lw=2.5,
                        color=color_list[3+eta_C_star_idx], zorder=0)
    ...
    ymax *= 0.85
    axes[2, idx].vlines(am_star.val, ymin, ymax, linestyle='dashed', lw=2.5,
                        color=color_list[3+eta_C_star_idx], zorder=0)
...
for eta_idx, eta in enumerate(eta_stars):
    ...
    label = r'$M_{\eta_C}^\star='+str(eta)+'$ GeV'\
            if eta != eta_PDG.val else r'$M_{\eta_C}^{\star,phys}$'

    ax1.plot(asq_grain, m_C_mSMOM_grain.val,
             linestyle='dashed',
    ...
    m_C_mSMOM_phys = m_C_mSMOM_mapping(0.0)
    ax1.errorbar([0], m_C_mSMOM_phys.val,
                 yerr=m_C_mSMOM_phys.err,
                 fmt='o', capsize=4,
                 color=color_list[3+eta_idx])
...
fig.savefig(
    '/Users/rajnandinimukherjee/Desktop/m_c.pdf')
...
plt.xlim(ren_xaxes)
plt.savefig(
    '/Users/rajnandinimukherjee/Desktop/mbar.pdf')
