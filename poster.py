...
mNPR_dict = {ens: mNPR(ens, mu=mu_chosen, mres=mres) for ens in ens_list}

filename = "/Users/rajnandinimukherjee/Desktop/poster_plots.pdf"
...
plt.axvspan(M1.am_C.val - M1.am_C.err, M1.am_C.val + M1.am_C.err, color="k", alpha=0.1)
plt.vlines(
    M1.am_C.val,
    0,
    eta_PDG.val + eta_PDG.err,
...
plt.text(M1.am_C.val * 1.01, 0.3, r"$am_C$", rotation=270, fontsize=18)
plt.ylim([ymin, ymax])
plt.xlim([-0.05, xmax])
plt.xlabel(r"$am$", fontsize=18)
plt.ylabel(r"$M_{\eta_C}$ (GeV)", fontsize=18)
...
plt.axhspan(
    M1.Z_SMOM.val - M1.Z_SMOM.err, M1.Z_SMOM.val + M1.Z_SMOM.err, color="k", alpha=0.1
)
...
for key in ["mam_q", "m"]:
    ...
    for ens in ["C1", "M1", "F1S"]:
        ...
        ax.axhspan(SMOM.val - SMOM.err, SMOM.val + SMOM.err, color="k", alpha=0.1)
...
for idx, ens in enumerate(["C1", "M1", "F1S"]):
    ...
    if ens == "C1":
        ...
        ens_name = "Fine"
    axes[0, idx].set_title(ens_name, fontsize=18)

    xmin, xmax = axes[0, idx].get_xlim()
    xmax = max(e.am_C.val * 1.05, xmax)
    axes[0, idx].hlines(
        eta_C_star, xmin, xmax, lw=2.5, color=color_list[3 + eta_C_star_idx]
    )

    axes[0, idx].hlines(eta_PDG.val, xmin, xmax, color="0.5", lw=2.5)
    axes[0, idx].axhspan(
    ...
    axes[0, idx].vlines(
        e.am_C.val, ymin, eta_PDG.val, linestyle="dashed", lw=2.5, color="0.5"
    )
    axes[0, idx].fill_between(
    ...
    axes[2, idx].set_xlabel("$a_{" + ens[0] + "}m$", fontsize=18)
...
axes[0, 0].set_ylabel(r"$M_{\eta_c}$ (GeV)", fontsize=20)
axes[0, 0].text(
    axes_info_combined[0, 0, 0] + 0.02,
    eta_C_star * 1.06,
    r"$M_{\eta_c}^\star$",
    fontsize=16,
)
axes[0, 0].text(
    axes_info_combined[0, 0, 0] + 0.02, eta_PDG.val * 1.04, r"phys", fontsize=16
)
axes[2, 0].set_ylabel(r"$Z_m^{mSMOM} \cdot m$ (GeV)", fontsize=20)
axes[1, 0].set_ylabel(r"$Z_m^{mSMOM} \cdot m_c$ (GeV)", fontsize=20)
plt.savefig("/Users/rajnandinimukherjee/Desktop/interpolation.pdf")
...
ax2 = ax1.twiny()
ax1.axvline(x=0, linestyle="dashed", color="k", alpha=0.4)

extrap.load_SMOM()
m_C_SMOM_mapping = extrap.extrap_mapping(extrap.m_C_SMOM, fit="quad")
m_C_SMOM_grain = m_C_SMOM_mapping(asq_grain)
ax1.plot(
    asq_grain, m_C_SMOM_grain.val, linestyle="dashed", color="0.5", label="massless"
)
ax1.fill_between(
    asq_grain,
...
ax1.set_xlabel(r"$a^2$ (GeV${}^{-2}$)", fontsize=14)
ax1.set_ylabel(r"$m_{C}^{ren}$ (GeV)", fontsize=14)
ax1.set_xlim(ren_xaxes)
ax2.set_xlim(ren_xaxes)
ax2.set_xticks(extrap.a_sq.val)
ax2.set_xticklabels(["C", "M", "F"])
fig.savefig("/Users/rajnandinimukherjee/Desktop/m_c.pdf")
...
plt.xlabel(r"$a^2$ (GeV${}^{-2}$)", fontsize=14)
plt.ylabel(r"$\overline{m}$ (GeV)", fontsize=14)
plt.xlim(ren_xaxes)
plt.savefig("/Users/rajnandinimukherjee/Desktop/mbar.pdf")
