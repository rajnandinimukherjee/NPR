...
if run:
    ...
    record_vals = {
        ...
        "C2M3cut": {
            "kwargs": {
                "ens_list": [b for b in bag_ensembles if b != "C2" and b != "M3"],
                "plot": False,
                "log": True,
                "addnl_terms": "del_ms",
                "guess": [1, 1e-1, 1e-2, 1e-2],
    ...
    for op_idx, op in enumerate(b.operators):
        ...
        for fit in record_vals.keys():
            record_vals[fit][f"B{op_idx+1}"] = b.fit_operator(
                mu,
                op,
                rotate=NPR_to_SUSY,
                ...
                figsize=(10, 3),
                label="",
                legend_axis=laxis[op_idx + 1],
                filename=filename,
                **record_vals[fit]["kwargs"],
            )
    ...
    for op_idx, op in enumerate(r.operators):
        ...
        for fit in record_vals.keys():
            record_vals[fit][f"R{op_idx+2}"] = r.fit_operator(
                mu,
                op,
                rotate=NPR_to_SUSY,
                chiral_extrap=chiral_extrap,
...
other_fit_filename = (
    f"fit_systematics_{str(int(mu*10))}_{other_fitter}_{scheme}{expand_str}.p"
)
...
rv += [r"\hline"]
rv += [
    r" & $m_\pi<420$ MeV  & $-\gamma\delta_{m_s}$ & $-L(m_\pi^2)$  "
    + r" & $m_\pi<440$ MeV & $m_\pi<370$ MeV & $m_\pi<350$ MeV & "
    + other_fitter
    + r" fit\\"
]
rv += [
    r" & central value & $\delta$ &  $\delta^\text{chiral}$ &"
...
for key in quantities.keys():
    ...
    central_str = (
        r"\textcolor{" + central_color + r"}{$" + err_disp(Y_0.val, Y_0.err) + r"$}"
    )
    ...
    no_gamma_change_str = (
        ...
        + no_gamma_color
        + r"}{$"
        + "{0:.2f}".format(no_gamma_change)
        + r"$\%}"
    )

    noC2cut_fit = record_vals["noC2cut"][key]
    noC2cut_change = np.abs(
        ((noC2cut_fit[0] - Y_0) / ((Y_0 + noC2cut_fit[0]) * 0.5)).val * 100
    )
    ...
    noC2cut_change_str = (
        ...
        + noC2cut_color
        + r"}{$"
        + "{0:.2f}".format(noC2cut_change)
        + r"$\%}"
    )

    C2M3cut_fit = record_vals["C2M3cut"][key]
    C2M3cut_change = np.abs(
        ((C2M3cut_fit[0] - Y_0) / ((Y_0 + C2M3cut_fit[0]) * 0.5)).val * 100
    )
    ...
    C2M3cut_change_str = (
        ...
        + C2M3cut_color
        + r"}{$"
        + "{0:.2f}".format(C2M3cut_change)
        + r"$\%}"
    )

    C2M3M2cut_fit = record_vals["C2M3M2cut"][key]
    C2M3M2cut_change = np.abs(
        ((C2M3M2cut_fit[0] - Y_0) / ((Y_0 + C2M3M2cut_fit[0]) * 0.5)).val * 100
    )
    ...
    C2M3M2cut_change_str = (
        ...
        + C2M3M2cut_color
        + r"}{$"
        + "{0:.2f}".format(C2M3M2cut_change)
        + r"$\%}"
    )

    no_log_fit = record_vals["no_log"][key]
    no_log_change = np.abs(
        ((Y_0 - no_log_fit[0]) / ((Y_0 + no_log_fit[0]) * 0.5)).val * 100
    )
    ...
    no_log_change_str = (
        ...
        + no_log_color
        + r"}{$"
        + "{0:.2f}".format(np.abs(no_log_change))
        + r"$\%}"
    )
    if key == "R2" or key == "R3":
        no_log_change_str = "-"
    ...
    other_change = np.abs(
        ((other_fit[0] - Y_0) / ((Y_0 + other_fit[0]) * 0.5)).val * 100
    )
    other_change_str = (
        r"\textcolor{" + other_color + r"}{$" + "{0:.2f}".format(other_change) + r"$\%}"
    )
