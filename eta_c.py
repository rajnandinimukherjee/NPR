...
class etaCvalence:
    ...
    def __init__(self, ens, create=False, **kwargs):
        ...
        self.mass_comb.update(
            {(f"c{m-1}", f"c{m-1}"): self.NPR_masses[m] for m in range(1, self.N_mass)}
        )
        self.data = {key: {} for key in self.mass_comb.keys()}
        self.eta_C_file = h5py.File(self.eta_C_filename, "r")

        if ens not in self.eta_C_file or create:
            self.eta_C_file = h5py.File(self.eta_C_filename, "a")
            self.createH5()
        else:
            self.N_cf, self.T = self.eta_C_file[self.ens][
                str(list(self.mass_comb.keys())[0])
            ]["corr"].shape
            self.data = {
                k: {
                    obj: {"data": np.zeros(shape=(self.N_cf, self.T), dtype=float)}
        ...
        for key in self.data.keys():
            for obj in ["corr", "PJ5q"]:
                ...
                err = np.array(
                    [st_dev(data[:, t], mean=avg_data[t]) for t in range(self.T)]
                )
                btsp_expanded = bootstrap(data, K=N_boot).real

                folded_data = (
                    0.5 * (data[:, 1:] + data[:, ::-1][:, :-1])[:, : self.T_half]
                )
                folded_avg_data = np.mean(folded_data, axis=0)
                if data.shape[0] == 1:
                    folded_err = np.array(
                        [folded_avg_data[t] * 0.0001 for t in range(self.T_half)]
                    )
                    btsp_data = np.array(
                        [
                            np.random.normal(folded_avg_data[t], folded_err[t], N_boot)
                            for t in range(self.T_half)
                        ]
                    ).T
                else:
                    folded_err = np.array(
                        [st_dev(folded_data[:, t]) for t in range(self.T_half)]
                    )
                    btsp_data = bootstrap(folded_data, K=N_boot).real

                cov = COV(btsp_data, center=folded_avg_data)

                self.data[key][obj].update(
                    {
                        "avg": avg_data,
                        "err": err,
                        "folded": folded_data,
                ...
                if obj == "corr":
                    ...
                    m_btsp = np.array(
                        [m_eff(btsp_data[b, :])[-2] for b in range(N_boot)]
                    )
                    self.data[key]["mccbar_err"] = st_dev(m_btsp, meff[-2])

            ratio = self.data[key]["PJ5q"]["avg"] / self.data[key]["corr"]["avg"]
            btsp_ratio = np.array(
                [
                    self.data[key]["PJ5q"]["btsp_exp"][k, :]
    ...
    def plot(self, keys="all", plotfits=False, **kwargs):
        ...
        for k in keylist:
            ...
            btsp = np.array(
                [
                    m_eff(self.data[k]["corr"]["btsp"][b, :], ansatz="cosh")
            ...
            if plotfits:
                ...
                plt.fill_between(
                    x_fit, y_plus, y_minus, color=color_list[mass_idx], alpha=0.5
                )
        ...
        plt.xlabel(r"$t$")
        plt.ylabel(
            r"$am_{res} = \langle 0|J_{5q}|\pi\rangle/" + r"\langle 0|J_{5}|\pi\rangle$"
        )
        plt.legend()
    ...
    def objfit(self, key, t_info, obj="corr", **kwargs):
        ...
        def diff(params, fit="central", k=0, **kwargs):
            if fit == "central":
                y = self.data[key]["corr"]["folded_avg" if obj == "corr" else "avg"][t]
            else:
                y = self.data[key][obj]["btsp"][k, t]
            return y - self.ansatz(params, t, obj=obj)

        cov = self.data[key][obj]["COV"][start : end + 1 : thin, start : end + 1 : thin]
        ...
        guess = [1e2, 0.1] if obj == "corr" else [0, 0.003]
        res = least_squares(LD, guess, args=("central", 0), ftol=1e-10, gtol=1e-10)
        ...
        params_err = np.array(
            [
                st_dev(params_btsp[:, i], mean=params_central[i])
            ...
            for c in range(self.N_cf):
                for obj in ["corr", "PJ5q"]:
                    ...
                    if obj == "corr":
                        ...
                        filepath = (
                            glob.glob(filepath + f"{key[0]}_R*{key[1]}_R*")[0] + "/"
                        )
                    ...
                    for t in range(T_src):
                        ...
                        datafile = (
                            h5py.File(filename, "r")["meson"][f"meson_{self.eta_C_idx}"]
                            if obj == "corr"
                            else h5py.File(filename, "r")["wardIdentity"]
                        )
                        data = np.array(datafile[obj])["re"]
                        t_val = int(
                            filename.rsplit("/")[-1].rsplit("_t")[1].rsplit("_")[0]
                        )
