...
am_dict = {
    ens: "{:.4f}".format(params[ens]["masses"][sea_mass_dict[ens]])
...
Z_4q_files = {
    ens: glob.glob(
        f"{NG_path}/{fq_scheme}/Z_4q_{fq_scheme}_{ens}_am_{am_dict[ens]}*{N_boot}.h5"
    )
    for ens in data_ensembles
}
...
for ens in data_ensembles:
    ...
    for f in Z_4q_files[ens]:
        ...
        bootstrap = np.moveaxis(
            np.array(data["bootstraps"][:]).reshape(5, 5, N_boot), -1, 0
        )
...
Z_AV_files = {
    ens: glob.glob(
        f"{NG_path}/{A_scheme}/Lambda_bil_VA_{A_scheme}_{ens}_am_{am_dict[ens]}*{N_boot}.h5"
    )
    for ens in data_ensembles
}
Z_SP_files = {
    ens: glob.glob(
        f"{NG_path}/g/Lambda_bil_SVTAP_g_{ens}_am_{am_dict[ens]}*{N_boot}.h5"
    )
    for ens in data_ensembles
}
...
for ens in data_ensembles:
    ...
    Z_bl[ens]["ap"] = momenta
    Z_bl[ens]["Z"] = {
        c: [ens_dict[m][c] for m in momenta] for c in ["S", "P", "V", "A"]
    }
...
for ens in data_ensembles:
    ...
    filename += "__".join(
        [
            "NPR",
            ens,
            params[ens]["baseactions"][a1],
            params[ens]["baseactions"][a2],
            scheme,
        ]
    )
    filename += ".h5"
    ...
    Z = stat(
        val=np.array([Z_ij_A.val[m,] * Z_A_2.val[m] for m in range(len(Z_A_2.val))]),
        err="fill",
        btsp=np.array(
            [
                [Z_ij_A.btsp[k, m] * Z_A_2.btsp[k, m] for m in range(len(Z_A_2.val))]
