...
def Bcoeffs(f, **kwargs):
    ...
    b4 += (
        (-336460813 - 115467936 * zeta(3) + 10994940 * zeta(4) + 97847640 * zeta(5))
        * f
        / 1944
    )
    b4 += (
        (25960913 + 16764744 * zeta(3) - 2273616 * zeta(4) - 9162240 * zeta(5))
        * (f**2)
        / 1944
    )
    b4 += (
        (-630559 - 1169328 * zeta(3) + 349488 * zeta(4) + 298080 * zeta(5))
        * (f**3)
        / 5832
    )
...
def R_mSMOM_to_MSbar(mu, mbar, mu_hat=None, f=4):
    ...
    if mbar != 0.0:
        ...
        return 1 + mult * CF * one_loop
    else:
        two_loop = 22.60768757 - 4.013539570 * f
        return 1 + mult * CF * one_loop - (mult**2) * two_loop
...
def MSbar_m_running(mu_start, mu_end, N_f=3, **kwargs):
    ...
    gamma_2_m = (
        3747 / 4 - (N_f * (1108 / 9 + (70 * N_f) / 27 + 80 * zeta(3))) / 2
    ) / 48
    gamma_3_m = (
        4603055 / 41472
        - (91723 * N_f) / 6912
    ...
    gammas = (
        np.array([gamma_0_m, gamma_1_m, gamma_2_m, gamma_3_m, gamma_4_m]) / betas[0]
    )
    betas = betas / betas[0]
    ...
    def running(a):
        ...
        return (a ** gammas[0]) * (
            1
            + (gammas[1] - betas[1] * gammas[0]) * a
            + (
                (gammas[1] - betas[1] * gammas[0]) ** 2
                + gammas[2]
                - betas[1] * gammas[1]
                + (betas[1] ** 2) * gammas[0]
                - betas[2] * gammas[0]
            ...
            * (a**2)
            / 2
            + (
                (gammas[1] - betas[1] * (gammas[0]) ** 3) / 6
                + (gammas[1] - betas[1] * gammas[0])
                * (
                    gammas[2]
                    - betas[1] * gammas[1]
                    + (betas[1] ** 2) * gammas[0]
...
r_gg_44 += z * (
    -C_0 * (1 / (2 * N) + (1 / 4)) - (N / 2) + 1 / (2 * N) + 2 * np.log(2) / N + (1 / 2)
)
r_gg_45 = 4 * (
    (C_0 / 8) * ((1 / N) - (1 / 2))
    - 7 / (6 * N)
...
r_gg_45 += (
    4
    * z
    * ((C_0 / 16) - 1 / (12 * N) + np.log(2) / (6 * N) + (1 / 24) - (np.log(2) / 3))
)
...
r_qq_45 = (
    ...
    + 2 * (1 - np.log(2)) / 3
    + z
    * (C_0 * (1 / N + 5 / 2) / 24 + (-1 + np.log(2)) / (6 * N) - 1 / 24 - np.log(2) / 3)
)
r_qq_45 = 4 * r_qq_45
r_qq_54 = (
    C_0 * (4 * N + 4 / N + 11)
...
def gamma_1_MS(f):
    ...
    g_45 = (
        -(N_c / 36) - (31 / 9) + (9 / N_c) - 4 / (N_c**2) + (f / 18) * ((2 / N_c) - 1)
    )
    g_54 = (
        -(364 * N_c / 3)
        - (704 / 3)
...
beta_0 = Bcoeffs(3)[0]
gamma_1_RISMOM = (
    r_gg_mtx @ gamma_0 - gamma_0 @ r_gg_mtx + gamma_1_MS(f=3) - 2 * beta_0 * r_gg_mtx
)
