...
NLO_3_to_inf = expm(-gamma_0*np.log(alpha_s(3.0,f=N_flav))/(2*Bcoeffs(N_flav)[0]))
NLO_term = Bcoeffs(N_flav)[1]*gamma_0 - Bcoeffs(N_flav)[0]*gamma_1_MS(N_flav)
NLO_term = NLO_term/(2*Bcoeffs(N_flav)[0]**2)
NLO_3_to_inf = NLO_3_to_inf@(np.identity(5)+(NLO_term)*(alpha_s(3.0,f=N_flav)/(4*np.pi)))
...
UKQCD16 = {'ratios':{'val':[-19.48, 6.08, 43.11, 10.99],
...
for idx in range(4):
    for fitter in [UKQCD16, SWME15, ETM15, ETM12]:
        fitter[f'B{idx+2}'] = stat(
                ...
                err=(fitter['bags']['err'][idx]**2+fitter['bags']['sys'][idx]**2)**0.5
                ) 
        fitter[f'B{idx+2}'].disp = err_disp(fitter['bags']['val'][idx],
                                            fitter['bags']['err'][idx],
                                            sys_err=fitter['bags']['sys'][idx])

    FLAG21[f'B{idx+2}'] = stat(
            val=FLAG21['bags']['val'][idx],
            err=FLAG21['bags']['err'][idx]
            )
    FLAG21[f'B{idx+2}'].disp = err_disp(FLAG21['bags']['val'][idx],
                                        FLAG21['bags']['err'][idx])
    UKQCD16[f'R{idx+2}'] = stat(
            val=UKQCD16['ratios']['val'][idx],
            err=(UKQCD16['ratios']['err'][idx]**2+UKQCD16['ratios']['sys'][idx]**2)**0.5
            )
    UKQCD16[f'R{idx+2}'].disp = err_disp(UKQCD16['ratios']['val'][idx],
...
fig.legend(handles[1:]+[handles[0]], labels[1:]+[labels[0]], loc='center right', 
           bbox_to_anchor=(1.45,0.49), labelspacing=1.55, frameon=False)
