...
mu = 2.0
linear_Z = M1M.fit_momentum_dependence(
        mu, masses, 'm', fittype='linear')
quadratic_Z = M1M.fit_momentum_dependence(
        mu, masses, 'm', fittype='quadratic')
...
fig, ax = plt.subplots(nrows=2, ncols=1,
                       sharex=True, figsize=(3,7.5))
plt.subplots_adjust(hspace=0)
x = join_stats([M1M.valence.amres[m]+eval(m)
                for m in M1M.all_masses[:-2]])
ax[1].set_xlabel(r'$am_q + am_\mathrm{res}$')

M_eta = join_stats([M1M.valence.eta_h_masses[m]*M1M.ainv
                for m in M1M.all_masses[:-2]])
ax[0].errorbar(x.val, M_eta.val,
               xerr=x.err, yerr=M_eta.err,
               capsize=4, fmt='o')
...
Z_m = join_stats([M1M.interpolate(mu, (m,m), 'm')
            for m in M1M.all_masses[:-2]])
ax[1].errorbar(x.val, Z_m.val,
               xerr=x.err, yerr=Z_m.err,
               capsize=4, fmt='o')
...
fig, ax = plt.subplots(ncols=len(ens_list),
                       sharey=True,
                       figsize=(3*len(ens_list),4))
...
for ens in reversed(ens_list):
    ...
    x = join_stats([e.valence.amres[m]+eval(m)
                    for m in e.all_masses[:-stop]])
    ax[i].set_xlabel(r'$am_q + am_\mathrm{res}$')

    M_eta = join_stats([e.valence.eta_h_masses[m]*e.ainv
                    for m in e.all_masses[:-stop]])
    ax[i].errorbar(x.val, M_eta.val,
                xerr=x.err, yerr=M_eta.err,
                capsize=4, fmt='o')
    ...
    xrange = np.linspace(x.val[0], xmax, 100)
    yrange = res.mapping(xrange)
    ax[i].fill_between(xrange, yrange.val+yrange.err,
             yrange.val-yrange.err,
             color='k', alpha=0.1)
    ...
    ax[i].hlines(y=eta_pdg.val, xmin=0,
                 xmax=xmax if i!=2 else am_c.val,
                 color='k')
    ax[i].fill_between(
            xrange if i!=2 else np.linspace(0,am_c.val,100),
                    ...
                    eta_pdg.val-eta_pdg.err,
                    color='k', alpha=0.2)
    ax[i].vlines(x=am_c.val, ymin=ymin, ymax=eta_pdg.val,
              color='k', linestyle='dashed')
    ax[i].text(am_c.val, 0.5, r'$am_c$', rotation=90,
            va='center', ha='right', color='k')
    ax[i].fill_between(np.linspace(am_c.val-am_c.err,
                                am_c.val+am_c.err,100),
                    ymin, eta_pdg.val+eta_pdg.err,
                    color='k', alpha=0.2)

    ax[i].hlines(y=eta_star.val, xmin=0,
                 xmax=xmax if i!=2 else am_star.val,
                 color='r')
    ax[i].fill_between(
            xrange if i!=2 else np.linspace(0,am_star.val,100),
                    ...
                    eta_star.val-eta_star.err,
                    color='r', alpha=0.2)
    ax[i].vlines(x=am_star.val, ymin=ymin, ymax=eta_star.val,
              color='r', linestyle='dashed')
    ax[i].text(am_star.val, 0.5, r'$am^\star$', rotation=90,
            va='center', ha='right', color='r')
    ax[i].fill_between(np.linspace(am_star.val-am_star.err,
    ...
    if i==0:
        ...
        ax[i].text(0.05, eta_pdg.val-0.05,
                r'$M_i=\frac{3}{4}M_{\eta_c}^\mathrm{PDG}$',
                va='top', ha='center', color='k')
        ax[i].text(0.05, eta_star.val-0.05,
...
for i, ens in enumerate(ens_list):
    ...
    Z_m = join_stats([e.interpolate(mu, (m,m), 'm')
                for m in e.all_masses[:-stop]])
    ax[i].errorbar(x.val, Z_m.val,
                xerr=x.err, yerr=Z_m.err,
                capsize=4, fmt='o')
    ...
    ax[i].set_title(ens)
    ymin, ymax = ax[i].get_ylim()
    ax[i].errorbar([am_star.val], [Z_m_amstar.val],
                xerr=[am_star.err], yerr=[Z_m_amstar.err],
                fmt='o', capsize=4, color='r')
    ax[i].vlines(x=am_star.val, ymin=ymin, ymax=ymax,
              color='r', linestyle='dashed')
    ax[i].fill_between(np.linspace(am_star.val-am_star.err,
                                am_star.val+am_star.err,100),
                    ymin, eta_star.val+eta_star.err,
    ...
    if i==0:
        ax[i].set_ylabel(r'$Z_m^\mathrm{mSMOM}(\mu='+str(mu)+\
                r'\,\mathrm{GeV})$')
    if i==len(ens_list)-1:
        ax[i].text(1.05, 0.5,
           r'$\alpha/am + \beta + \gamma\,am + \delta\,(am)^2$',
...
ax.legend()
ax.set_title(r'$M_i=M_{\eta_c}^\mathrm{PDG},\,'+\
...
frac_labels = [r'\frac{2}{5}', r'\frac{1}{2}', r'\frac{3}{5}',
               r'\frac{3}{4}', r'\frac{9}{10}', r'']
...
ax.fill_between(xrange, yrange.val+yrange.err,
            ...
            color='k', alpha=0.1)
ax.set_xlim([xmin, xmax])
ax.text(0.5, 0.1, r'$\chi^2/\mathrm{DOF}:'+\
        str(np.around(res.chi_sq/res.DOF, 3))+r'$',
        ha='center', va='center',
        transform=ax.transAxes)
ax.text(0.1, 0.9, r'$\mu=2\,\mathrm{GeV}$',
        ha='left', va='center',
        transform=ax.transAxes)
ax.text(0.1, 0.8, 
        r'$\overline{M}=\frac{3}{4}M_{\eta_c}^\mathrm{PDG}$',
...
for eta_idx, eta_star in enumerate(M_star_list):
    label = r'$\overline{M}='+frac_labels[eta_idx]+\
    ...
    res_mc_mSMOM = fit_func(x, y_mc_mSMOM, ansatz,
                            guess, end=end)
    y_mc_mSMOM_phys = res_mc_mSMOM[0]
