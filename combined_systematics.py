...
errors_dict = {
    "gamma": {"name": r"$\textrm{RI-SMOM}^{(\gamma_\mu,\gamma_\mu)}$"},
...
for scheme in errors_dict.keys():
    fit_systematics = pickle.load(
        open(f"fit_systematics_20_{fit_file}_{scheme}{expand_str}.p", "rb")
    )
    scaling_systematics = pickle.load(
        open(f"scaling_systematics_{scheme}_{fit_file}{expand_str}.p", "rb")
    )
    other_systematics = pickle.load(
        open(f"other_systematics_{scheme}_{fit_file}{expand_str}.p", "rb")
    )
    for key in quantities.keys():
        ...
        chiral_fits = [
            fit_systematics[fittype][key][0]
            for fittype in ["no_del_ms", "C2M3cut", "C2M3M2cut", "no_log"]
        ]
        delta_chirals = [
            np.abs(
                ((chiral_fit - central_2) / ((chiral_fit + central_2) * 0.5)).val * 100
            )
        ...
        discr_fits = [
            scaling_systematics[fittype][key]
            for fittype in ["(3)", "(2,3,0.5)", "(2,3,0.33)"]
        ]
        delta_discrs = [
            np.abs(((discr_fit - central) / ((discr_fit + central) * 0.5)).val * 100)
        ...
        total_err = (
            central_perc_err**2 + delta_chiral**2 + delta_discr**2 + delta_rcsb**2
        ) ** 0.5
        total_err_str = "{0:.2f}".format(total_err) + r"\%"

        num_digits = (
            int(np.floor(np.abs(np.log10(np.abs(total_err * central.val / 100))))) + 2
        )
        ...
        errors_dict[scheme][key] = {
            "central": central_val_str,
            "stat": stat_err_str,
        ...
        RISMOM_results[scheme][key] = {
            ...
            "rcsb": stat(
                val=central.val, err=np.abs(central.val * delta_rcsb) / 100, btsp="fill"
            ),
            "discr": stat(
                val=central.val,
            ...
            "total": stat(
                val=central.val, err=np.abs(total_err * central.val / 100), btsp="fill"
            ),
        }

MS_bar_results = {
    "gamma": {
        "name": r"$\overline{\text{MS}}\leftarrow\text{RI-SMOM}^{(\gamma_\mu, \gamma_\mu)}$"
    },
    "qslash": {
        "name": r"$\overline{\text{MS}}\leftarrow\text{RI-SMOM}^{(\slashed{q}, \slashed{q})}$"
    },
...
for scheme in MS_bar_results.keys():
    ...
    if scheme in ["gamma", "qslash"]:
        ...
        R_conv_bag = (
            np.diag(1 / N_i)
            @ R_RISMOM_MSbar(3.0, scheme=scheme, obj="bag", rotate=NPR_to_SUSY)
            @ np.diag(N_i)
        )
        ...
        for err_type in ["central", "chiral", "rcsb", "discr"]:
            bags = join_stats(
                [RISMOM_results[scheme][f"B{idx+1}"][err_type] for idx in range(5)]
            )
            MS_bag = R_conv_bag @ bags
            for idx in range(5):
                MS_bar_results[scheme][f"B{idx+1}"][err_type] = MS_bag[idx]

        R_conv_rat = R_RISMOM_MSbar(3.0, scheme=scheme, obj="ratio", rotate=NPR_to_SUSY)
        R_conv_rat = stat(val=R_conv_rat, err=np.zeros((5, 5)), btsp="fill")

        for err_type in ["central", "chiral", "rcsb", "discr"]:
            ratios = join_stats(
                [stat(val=1, err=0, btsp="fill")]
        ...
        for key in quantities.keys():
            ...
            chiral_perc_err = np.abs(
                MS_bar_results[scheme][key]["chiral"].err * 100 / central.val
            )
            chiral_err_str = "{0:.2f}".format(chiral_perc_err) + r"\%"

            rcsb_perc_err = np.abs(
                MS_bar_results[scheme][key]["rcsb"].err * 100 / central.val
            )
            rcsb_err_str = "{0:.2f}".format(rcsb_perc_err) + r"\%"

            discr_perc_err = np.abs(
                MS_bar_results[scheme][key]["discr"].err * 100 / central.val
            )
            ...
            if key == "B5" and scheme == "qslash":
                num_digits += 1
            central_val_str = (
                r"$" + ("{0:.%df}" % num_digits).format(central.val) + r"$"
            )

            MS_bar_results[scheme][key]["str"] = {
                "central": central_val_str,
                "stat": stat_err_str,
                "chiral": chiral_err_str,
                "rcsb": rcsb_err_str,
        ...
        for key in quantities.keys():
            ...
            key_stat = stat(
                val=central, err=(stat_err**2 + total_sys_err**2) ** 0.5, btsp="fill"
            )
            key_stat.disp = err_disp(
                key_stat.val, stat_err, sys_err=np.abs(total_sys_err)
            )
            ...
            key_stat.alt_disp = err_disp(
                key_stat.val, lat_err, sys_err=np.abs(PT_err_perc * central / 100)
            )
            ...
            MS_bar_results[scheme][key]["str"] = {
                "central": central_val_str,
                r"$(\gamma_\mu,\gamma_\mu)$": MS_bar_results["gamma"][key]["str"][
                    "central"
                ],
                r"$(\slashed{q},\slashed{q})$": MS_bar_results["qslash"][key]["str"][
                    "central"
                ],
                "lattice": lat_err_str,
                "PT": PT_err_str,
...
for scheme in errors_dict.keys():
    rv += [
        r"\multirow{6}{*}{"
        + errors_dict[scheme]["name"]
        + r"} & central & "
        + " & ".join(
            [errors_dict[scheme][key]["central"] for key in list(quantities.keys())]
        )
        + r" \\"
    ]
    rv += [r"\cline{2-11}"]
    for err in ["stat", "chiral", "rcsb", "discr", "total"]:
        ...
        rv += [
            ...
            + r" & "
            + " & ".join(
                [errors_dict[scheme][key][err] for key in list(quantities.keys())]
            )
            + r" \\"
        ]
    rv += [r"\hline"]
...
rv += [
    r"\multirow{6}{*}{"
    + MS_bar_results["combined"]["name"]
    + r"} & $(\gamma_\mu,\gamma_\mu)$ & "
    + " & ".join(
        [
            MS_bar_results["combined"][key]["str"][r"$(\gamma_\mu,\gamma_\mu)$"]
...
for err in [r"$(\slashed{q},\slashed{q})$", "central", "lattice", "PT", "total"]:
    rv += [
        ...
        + " & ".join(
            [
                MS_bar_results["combined"][key]["str"][err]
                for key in list(quantities.keys())
            ]
        )
        + r" \\"
...
for scheme in ["gamma", "qslash"]:
    rv += [
        r"\multirow{6}{*}{"
        + MS_bar_results[scheme]["name"]
        + r"} & central & "
        + " & ".join(
            [
                MS_bar_results[scheme][key]["str"]["central"]
                for key in list(quantities.keys())
            ]
        )
        + r" \\"
    ...
    for err in ["stat", "chiral", "rcsb", "discr", "total"]:
        ...
        rv += [
            ...
            + " & ".join(
                [
                    MS_bar_results[scheme][key]["str"][err]
                    for key in list(quantities.keys())
                ]
            )
            + r" \\"
...
for key in ["gamma", "qslash"]:
    ...
    RISMOM_BK_2GeV = pickle.load(
        open(f"fit_systematics_20_{fit_file}_{key}{expand_str}.p", "rb")
    )["central"]["B1"][0]
    BK_running = RISMOM_BK_2GeV / RISMOM_BK_3GeV["central"]
    MS_conv_factor = R_RISMOM_MSbar(2.0, scheme=key, obj="bag", rotate=NPR_to_SUSY)[
        0, 0
    ]
    print(
        f"{key}: sig(2,3): {err_disp(BK_running.val, BK_running.err)}, MS_conv:{MS_conv_factor}"
    )
    for error_key, BK in RISMOM_BK_3GeV.items():
        ...
        print(
            f"{key}, {error_key}: BK(3GeV): {err_disp(BK.val, BK.err)}, BK(2GeV): {err_disp(RISMOM_BK_2GeV_key.val, RISMOM_BK_2GeV_key.err)}, MS_BK(2GeV):{err_disp(MS_BK_2GeV[key][error_key].val, MS_BK_2GeV[key][error_key].err)}\n"
        )

BK_2GeV_central = (
    MS_BK_2GeV["gamma"]["central"].val + MS_BK_2GeV["qslash"]["central"].val
) / 2
MS_BK_2GeV["combined"] = {
    error_key: stat(
        val=BK_2GeV_central,
        err=(MS_BK_2GeV["gamma"][error_key].err / MS_BK_2GeV["gamma"][error_key].val)
        * BK_2GeV_central,
        btsp="seed",
        seed=1,
    )
...
breakdown = [
    err_disp(BK_2GeV_central, val.err) + error_key
