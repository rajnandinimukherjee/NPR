...
def fit_func(
    ...
    **kwargs,
    ...
    def LD(param):
        return L.dot(diff(x.val[start:end], y.val[start:end], param, fit="central"))
    ...
    for k in range(N_boot):
        ...
        def LD_k(param):
            return L.dot(
                diff(x.btsp[k, start:end], y.btsp[k, start:end], param, fit="btsp", k=k)
            )
    ...
    if pvalue < 0.05 and chi_sq_rescale:
        res = stat(val=res.val, err=res.err * ((chi_sq / DOF) ** 0.2), btsp="fill")
    ...
    def mapping(am):
        ...
        return stat(
            ...
            err="fill",
            btsp=np.array(
                [ansatz(am.btsp[k,], res.btsp[k], fit="recon") for k in range(N_boot)]
            ),
        )
...
class stat:
    ...
    def __init__(self, val, err=None, btsp=None, dtype=None, **kwargs):
        ...
        if type(err) == str:
            if err == "fill":
                ...
                percent = float(err[:-1])
                dist = np.random.normal(percent, percent / 4, size=self.val.shape)
    ...
    def calc_btsp(self, seed=None):
        ...
        for idx, central in np.ndenumerate(self.val):
            ...
            self.btsp[idx] = np.random.normal(central, self.err[idx], self.N_boot)
        self.btsp = np.moveaxis(self.btsp, -1, 0)

    def use_func(self, func, **kwargs):
        central = func(self.val, **kwargs)

        btsp = np.array([func(self.btsp[k,], **kwargs) for k in range(N_boot)])
    ...
    def __add__(self, other):
        ...
        new_stat = stat(
            ...
            err="fill",
            btsp=np.array([self.btsp[k,] + other.btsp[k,] for k in range(self.N_boot)]),
        )
        return new_stat
    ...
    def __getitem__(self, indices):
        key = indices
        new_stat = stat(val=self.val[key], err=self.err[key], btsp=self.btsp[:, key])
        return new_stat
...
UKQCD_ens = ["C0", "C1", "C2", "M0", "M1", "M2", "M3", "F1M", "F1S"]
...
flag_N = stat(
    ...
    err="fill",
    btsp=np.array(
        [norm_factors(rotate=NPR_to_SUSY) * flag.btsp[k,] for k in range(N_boot)]
    ),
)
...
# =====gamma matrices=============================================
gamma = {
    "I": np.identity(N_d, dtype="complex128"),
...
Gamma = {
    name: np.einsum("ab,cd->acbd", mtx, np.identity(N_c)).reshape((12, 12))
...
def bootstrap(data, seed=1, K=N_boot, **kwargs):
    ...
    if C == K:  # goes off when data itself is bootstrap data
        ...
        samples = np.mean(
            data[
                tuple(slicing.T if ax == 0 else slice(None) for ax in range(data.ndim))
            ],
            axis=1,
        )
...
def common_cf_files(data, cfgs, corr, prefix=None):
    file_names = {cf: os.listdir(f"{data}/{cf}/NPR/{corr}/") for cf in cfgs}
...
def decode_prop(prop_name):
    ...
    prop_info = {
        "prop": "_".join(info_list[:Ls_idx]),
        "Ls": info_list[Ls_idx + 1],
        "M5": info_list[info_list.index("M5") + 1],
        "am": info_list[info_list.index("am") + 1],
        "tw": "_".join(
            info_list[info_list.index("tw") + 1 : info_list.index("tw") + 5]
        ),
        "src_mom_p": "_".join(
            info_list[info_list.index("p") + 1 : info_list.index("p") + 5]
        ),
    }
    return prop_info
...
color_list = list(mc.TABLEAU_COLORS.keys()) + list(mc.BASE_COLORS.keys())
