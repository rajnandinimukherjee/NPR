...
def check_NPR(ens, pass_only=False):
    ...
    for cfg in cfgs:
        for mass in masses:
            for mom, tw in itertools.product(moms, twistvals):
                ...
                files = glob.glob(
                    f"{loc}/{cfg}/NPR/bilinears/*_{action}"
    ...
    if pass_only:
        ...
        print(
            f"Expecting (mom)x(tw) = {len(moms)}x{len(twistvals)} = {len(moms)*len(twistvals)} files per config per mass point\n"
        )
        print(df)
...
def check_valence(ens, data="mesons", pass_only=False):
    ...
    for cfg in cfgs:
        for mass in masses:
            ...
            if data == "mesons":
                ...
                files = glob.glob(
                    f"{loc}/{cfg}/mesons/{foldername}/meson_prop_*{cfg}.h5"
                )
            else:
                files = glob.glob(
                    f"{loc}/{cfg}/conserved/prop_{mass_names[mass]}_{R}_Z2_t*{cfg}.h5"
                )
    ...
    if pass_only:
        ...
        print(
            f"Expecting N_src = 16 (32 if sm data available) files per config per mass point\n"
        )
        print(df)
