...
ens_colors = {
    "C0": [1, 0, 0],
    "C1": [250.0 / 256, 128.0 / 256, 114.0 / 256],
    "C1M": [128.0 / 256, 128.0 / 256, 128.0 / 256],
    "C2": [240.0 / 256, 190.0 / 256, 128.0 / 256],
    "M0": [0, 0, 1.0],
    "M1": [30.0 / 256, 144.0 / 256, 1],
    "M1M": [128.0 / 256, 128.0 / 256, 128.0 / 256],
    "M2": [135.0 / 256, 206.0 / 256, 250.0 / 256],
    "M3": [185.0 / 256, 206.0 / 256, 256.0 / 256],
    "F1M": [154.0 / 256, 205.0 / 256, 50.0 / 256],
}
...
def chiral_continuum_ansatz(
    params, a_sq, m_f_sq, PDG, operator, obj="bag", log=False, **kwargs
):
...
class Z_fits:
    ...
    def __init__(
        self, ens_list, norm="V", mask=fq_mask.copy(), run_extrap=False, **kwargs
    ):
    ...
    def Z_assignment(self, mu, chiral_extrap=False, **kwargs):
        if not chiral_extrap:
            ...
            for e in self.ens_list:
                Z_linear = self.Z_dict[e].interpolate(mu, fittype="linear", **kwargs)
                ...
                stat_err = np.array(
                    [
                        [
                            max(Z_linear.err[i, j], Z_quadratic.err[i, j])
                ...
                sys_err = np.array(
                    [
                        [
                            np.abs(Z_linear.val[i, j] - Z_quadratic.val[i, j]) / 2
                            for j in range(self.N_ops)
                        ]
                        for i in range(self.N_ops)
                    ]
                    ...
                    if ens in CS:
                        ...
                        sys_err = np.array(
                            [
                                [
                                    np.abs(
                                        max(z.val[i, j] for z in all_4)
                                        - min(z.val[i, j] for z in all_4)
                                    )
                                    / 2
                                    for j in range(self.N_ops)
                                ]
                        ...
                        Z = stat(val=all_sum.val, err=stat_err, btsp="seed", seed=1)
                        Z.sys_err = sys_err
            ...
            for ens in self.ens_list:
                ...
                if ens == "F1M":
                    other_sys_error = np.array(
                        [
                            [
                                max(Z_linear.sys_err[i, j], Z_quadratic.sys_err[i, j])
                                for j in range(self.N_ops)
                            ]
                            for i in range(self.N_ops)
                        ]
    ...
    def Z_chiral_extrap(
        ...
        **kwargs,
        ...
        chiral_data = join_stats(
            [
                self.Z_dict[ens].interpolate(mu, fittype=fittype, **kwargs)
                for ens in ens_list
            ]
        )
        x = join_stats([self.Z_dict[ens].m_pi ** 2 for ens in ens_list])
        ...
        fit_params = stat(val=param_save, err="fill", btsp=param_save_btsp)
        extrap = stat(
            val=[
                [extrap[i, j].val for j in range(self.N_ops)] for i in range(self.N_ops)
            ],
            err="fill",
            btsp=[
                [
                    [extrap[i, j].btsp[k] for j in range(self.N_ops)]
                    for i in range(self.N_ops)
                ]
        ...
        def mapping(i, j, m_sq):
            if type(m_sq) is not stat:
                m_sq = stat(val=m_sq, btsp="fill")
            Z_map = stat(
                val=ansatz(m_sq.val, fit_params.val[:, i, j]),
                err="fill",
                btsp=[
                    ansatz(m_sq.btsp[k], fit_params.btsp[k, :, i, j])
                    for k in range(N_boot)
                ],
            )
            return Z_map
        ...
        **kwargs,
        ...
        sigmas = join_stats(
            [
                stat(
                    ...
                    btsp=np.array(
                        [
                            Z_dict_mu2[e].btsp[k] @ np.linalg.inv(Z_dict_mu1[e].btsp[k])
                            for k in range(N_boot)
                        ]
                    ),
                )
        ...
        for i, j in itertools.product(range(self.N_ops), range(self.N_ops)):
            if self.mask[i, j]:
                y = stat(
                    val=sigmas.val[:, i, j],
                    err=sigmas.err[:, i, j],
                    btsp=np.array([sigmas.btsp[k, :, i, j] for k in range(N_boot)]),
                )

                res = fit_func(
                    x, y, ansatz, [1, 1e-1], chi_sq_rescale=chi_sq_rescale, **kwargs
                )
                param_save[:, i, j] = res.val
                param_save_btsp[:, :, i, j] = res.btsp
                extrap[i, j] = res.mapping(0.0)
        ...
        def mapping(i, j, a_sq):
            if type(a_sq) is not stat:
                a_sq = stat(val=a_sq, btsp="fill")
            sig_map = stat(
                val=ansatz(a_sq.val, fit_params.val[:, i, j]),
                err="fill",
                btsp=[
                    ansatz(a_sq.btsp[k], fit_params.btsp[k, :, i, j])
                    for k in range(N_boot)
                ],
            )
            return sig_map
                ...
                for i, j in itertools.product(range(self.N_ops), range(self.N_ops)):
                    if self.mask[i, j]:
                        y = stat(
                            ...
                            err=sigmas.err[:, i, j],
                            btsp=np.array(
                                [sigmas.btsp[k, :, i, j] for k in range(N_boot)]
                            ),
                        )
                        ax[i, j].errorbar(
                            x.val, y.val, yerr=y.err, xerr=x.err, fmt="o", capsize=4
    ...
    def compare_scaling(
        ...
        **kwargs,
        ...
        if os.path.isfile(npt_filename):
            ...
            npt_scaling = join_stats(
                [
                    self.calc_running(mu_grain[m_idx], mu2, **kwargs)
        ...
        LO_scaling = np.array(
            [expm(gamma_0 * np.log(g(mu2) / g(m)) / Bcoeffs(3)[0]) for m in mu_grain]
        )
        NLO_scaling = np.array(
            [
                K(mu2) @ LO_scaling[m_idx] @ np.linalg.inv(K(mu_grain[m_idx]))
        ...
        for i, j in itertools.product(range(self.N_ops), range(self.N_ops)):
            if self.mask[i, j]:
                ...
                ax.plot(mu_grain, LO_scaling[:, i, j], "--", label="LO")
                ax.plot(mu_grain, NLO_scaling[:, i, j], "-", label="NLO")
                ...
                if separate:
                    call_PDF(
                        filename[:-4] + f"_{i+1}{j+1}" + filename[-4:], open=open_file
                    )
...
class bag_fits:
    ...
    def __init__(self, ens_list, obj="bag", **kwargs):
        ...
        self.bag_dict = {e: bag_analysis(e, obj=obj) for e in self.ens_list}
        self.Z_fits = Z_fits(self.ens_list, norm=self.norm, **kwargs)

    def load_bag(self, mu, chiral_extrap=False, **kwargs):

        self.Z_dict = self.Z_fits.Z_assignment(
            mu, chiral_extrap=chiral_extrap, **kwargs
        )
        if "rotate" in kwargs:
        ...
        rot_mtx = stat(val=rot_mtx, btsp="fill")

        self.mu = mu
        self.bag = join_stats(
            [self.Z_dict[e] @ (rot_mtx @ self.bag_dict[e].bag) for e in self.ens_list]
        )
    ...
    def fit_operator(
        ...
        **kwargs,
        ...
        if ens_list == None:
            ...
            ens_indices = [self.ens_list.index(ens) for ens in ens_list]
            bag = stat(
        ...
        def ansatz(x, param, **kwargs):
            p = [self.bag_dict[e].ansatz(param, operator, **kwargs) for e in ens_list]
            return np.array(p)

        y = stat(
            val=bag.val[:, op_idx], err=bag.err[:, op_idx], btsp=bag.btsp[:, :, op_idx]
        )
        ...
        if "log" in kwargs:
            if kwargs["log"]:
                log_term = chiral_logs(obj=self.obj, **kwargs)[op_idx] / (
                    (4 * np.pi) ** 2
                )
                log_term = (
                    m_f_sq_PDG
                    * ((m_pi_PDG**2) / (Lambda_QCD**2)).use_func(np.log)
                    * log_term
                )
        ...
        if expand_err:
            ...
            if print_F1M_recons:
                print(
                    f"{self.obj} {op_idx+1}", err_disp(F1M_recons.val, F1M_recons.err)
                )
        ...
        if plot:
            ...
            for ens_idx, e in enumerate(ens_list):
                ...
                mpi_phys = stat(
                    ...
                    err="fill",
                    btsp=[(m_pi.btsp[k] ** 2) / a_sq.btsp[k] for k in range(N_boot)],
                )
                ax[1].errorbar(
                    [mpi_phys.val],
                    [y_ens.val],
            ...
            ax[1].axvline(m_pi_PDG.val**2, color="k", linestyle="dashed")
            ax[0].axvline(0, color="k", linestyle="dashed")
            ...
            ax[0].text(
                0.4,
                0.05,
                r"$\chi^2$/DOF:" + "{:.3f}".format(res.chi_sq / res.DOF),
                transform=ax[0].transAxes,
            )
            ax[1].text(
                0.4,
                0.05,
                r"$p$-value:" + "{:.3f}".format(res.pvalue),
        ...
        if "log" in kwargs:
            if kwargs["log"]:
                res.val[0], res.err[0], res.btsp[:, 0] = (
                    y_phys.val,
                    y_phys.err,
                    y_phys.btsp,
                )
