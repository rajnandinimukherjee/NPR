...
if run:
    ...
    record_vals = {"(2,3)": {}, "(2,3,0.5)": {}, "(2,3,0.33)": {}, "(3)": {}}
    ...
    sig_bag = s_bag.calc_running(
        2.0, 3.0, chiral_extrap=True, rotate=NPR_to_SUSY, chi_sq_rescale=True
    )
    ...
    for stepsize in [1 / 3, 0.5]:
        ...
        sig_bag_steps = stat(
            val=np.eye(len(b.operators)),
            btsp=[np.eye(len(b.operators)) for k in range(N_boot)],
        )
        ...
        for i in range(len(bags.val)):
            record_vals[f"(2,3,{np.around(stepsize,2)})"][f"B{i+1}"] = (
                sig_bag_steps @ bags
            )[i] / N_i[i]
    ...
    for stepsize in [1 / 3, 0.5]:
        steps = np.arange(2.0, 3.0 + 0.1, stepsize)
        sig_rat_steps = stat(val=np.eye(5), btsp=[np.eye(5) for k in range(N_boot)])
        for i in range(len(steps) - 1):
            sig_rat_steps = (
                s_rat.calc_running(
                    steps[i],
        ...
        for i in range(1, len(ratios.val)):
            record_vals[f"(2,3,{np.around(stepsize,2)})"][f"R{i+1}"] = (
                sig_rat_steps @ ratios
            )[i]
...
for key in quantities.keys():
    ...
    central_str = (
        ...
        + central_color
        + r"}{$"
        + err_disp(central.val, central.err)
        + r"$}"
    )
    ...
    direct_disc_str = (
        r"\textcolor{" + direct_color + r"}{$" + "{0:.2f}".format(delta_disc) + r"$\%}"
    )
    ...
    no_mask_change = np.abs(
        ((no_mask - central) / ((no_mask + central) * 0.5)).val * 100
    )
    no_mask_change_str = (
        r"\textcolor{"
        + no_mask_color
        + r"}{$"
        + "{0:.2f}".format(no_mask_change)
        + r"$\%}"
    )
    ...
    NPR_change_str = (
        r"\textcolor{" + NPR_color + r"}{$" + "{0:.2f}".format(NPR_change) + r"$\%}"
    )
