...
fq_proj = {'SS': np.einsum('ab,cd->abcd', Gamma['I'], Gamma['I']),
           ...
           'AA': np.sum(np.array(
               [np.einsum('ab,cd->abcd',
                ...
                for i in dirs]), axis=0),
           'TT': np.sum(np.array(
               sum([[np.einsum('ab,cd->abcd',
                               Gamma[dirs[i]]@Gamma[dirs[j]],
                               Gamma[dirs[i]]@Gamma[dirs[j]])
                     for j in range(i+1, 4)]
                    for i in range(0, 4-1)], [])), axis=0)}

fq_gamma_projector = {'VVpAA': fq_proj['VV']+fq_proj['AA'],
                      'VVmAA': fq_proj['VV']-fq_proj['AA'],
                      'SSmPP': fq_proj['SS']-fq_proj['PP'],
                      'SSpPP': fq_proj['SS']+fq_proj['PP'],
                      'TT': fq_proj['TT']}
...
def fq_qslash_projector(q, p1, p2, **kwargs):
    ...
    proj = {'VV': np.einsum('ab,cd->abcd', qslash, qslash)/q_sq,
            'AA': np.einsum('ab,cd->abcd', qslash@gamma['5'],
                            qslash@gamma['5'])/q_sq}
    P_L = (np.identity(4)-gamma['5'])/2
    p_sig_P_p = np.sum([np.sum([p1[dirs.index(i)] *
                                ((commutator(i, j, g=gamma)/2)@P_L) *
                                p2[dirs.index(j)]
                                for j in dirs], axis=0)
                        for i in dirs], axis=0)
    proj['TT'] = np.einsum('ab,cd->abcd', p_sig_P_p,
                           p_sig_P_p)/(p1_sq*p2_sq - p1dotp2**2)

    q_proj = {'VVpAA': np.einsum('abcd,ef,gh->abcdefgh',
                                 proj['VV']+proj['AA'],
                                 np.identity(N_c),
                                 np.identity(N_c)),
              'VVmAA': np.einsum('abcd,ef,gh->abcdefgh',
                                 proj['VV']-proj['AA'],
                                 np.identity(N_c),
                                 np.identity(N_c))}
    q_proj['SSmPP'] = q_proj['VV-AA'].swapaxes(4, 6)
    q_proj['TT'] = np.einsum('abcd,ef,gh->abcdefgh',
                             proj['TT'],
                             np.identity(N_c),
                             np.identity(N_c))
    q_proj['TT'] = np.moveaxis(np.moveaxis(
        np.moveaxis(q_proj['TT'], 4, 1), 5, 3), 6, 5)
...
class fourquark:
    ...
    def __init__(self, ensemble, prop1, prop2, scheme='gamma', **kwargs):
        ...
        self.N_cf = len(cfgs)
        self.fourquark = np.array([np.empty(
            shape=(self.N_cf, 12, 12, 12, 12), dtype='complex128')
        ...
        for cf in range(self.N_cf):
            ...
            for i in range(N_fq):
                ...
                self.fourquark[i][cf, :] = np.array(
                    fourquark_i['re']+fourquark_i['im']*1j).\
                    swapaxes(1, 2).swapaxes(5, 6).\
                    reshape((12, 12, 12, 12))
    ...
    def NPR(self, **kwargs):
        self.gammas = [[[x for x in list(self.org_gammas[i][0])
                        ...
                        [y for y in list(self.org_gammas[i][1])
                         if y in list(gamma.keys())]]
                       for i in range(N_fq)]

        self.doublets = {'SS': self.fourquark[self.gammas.index([['I'], ['I']])],
                         'PP': self.fourquark[self.gammas.index([['5'], ['5']])],
                         'VV': np.sum(np.array(
                             [self.fourquark[self.gammas.index([[i], [i]])]
                              for i in dirs]), axis=0),
                         'AA': np.sum(np.array(
                             [self.fourquark[self.gammas.index(
                                 [[i, '5'], [i, '5']])]
                              for i in dirs]), axis=0),
                         'TT': np.sum(np.array(sum(
                             [[self.fourquark[self.gammas.
                                              index([[dirs[i], dirs[j]],
                                                     [dirs[i], dirs[j]]])]
                               for j in range(i+1, 4)]
                              for i in range(0, 4-1)], [])), axis=0)}
        self.operator = {'VVpAA': self.doublets['VV']+self.doublets['AA'],
