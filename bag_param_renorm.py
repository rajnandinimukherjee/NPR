...
class Z_analysis:
    ...
    def __init__(self, ensemble, action=(0, 0), bag=False, **kwargs):
        ...
        self.f_m_ren_btsp = np.random.normal(
            self.f_m_ren, f_pi_PDG_err/self.ainv, self.N_boot)
        ...
        if self.bag:
            ...
            for m in range(self.N_mom):
                ...
                for k in range(self.N_boot):
                    ...
                    self.Z_btsp[m, 1:, 1:, k] = self.Z_btsp[m,
                                                            1:, 1:, k]*(mult**2)
    ...
    def interpolate(self, mu, **kwargs):
        if mu in self.momenta:
            mu_idx = list(self.momenta).index(mu)
            matrix, errs, btsp = self.Z[mu_idx,
                                        ], self.Z_err[mu_idx,], self.Z_btsp[mu_idx,]
        else:
...
class bag_analysis:
    ...
    def rotate_bag(self, rot_mtx, bag, err, btsp, **kwargs):
        bag_rot = rot_mtx@bag
        btsp_rot = np.array([rot_mtx@btsp[k, :]
                            for k in range(self.N_boot)])
        err_rot = np.array([st_dev(btsp_rot[:, i], bag_rot[i])
                            for i in range(len(self.operators))])
        return bag_rot, err_rot, btsp_rot
    ...
    def interpolate(self, mu, **kwargs):
        ...
        err = np.array([st_dev(btsp[:, i], mean=central[i])
                        for i in range(5)])
        if 'rotate' in kwargs:
            return self.rotate_bag(kwargs['rotate'], central, err, btsp)
        else:
    ...
    def ansatz(self, params, operator, **kwargs):
