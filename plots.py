...
plt.errorbar(
    x, y, yerr=yerr, fmt="o", capsize=4, color=color_list[ens_list.index("M1")]
)
xmin, xmax = plt.xlim()
plt.xlim([-0.05, xmax])
...
plt.hlines(eta_PDG.val, -0.05, am_C, color=color_list[3 + eta_stars.index(eta_PDG.val)])
plt.axvspan(am_C - am_C_err, am_C + am_C_err, color="k", alpha=0.1)
plt.vlines(
    am_C,
    0,
    eta_PDG.val,
...
plt.text(am_C * 1.01, 0.3, r"$am_C$", rotation=270, fontsize=18)
plt.ylim([ymin, ymax])
plt.xlim([-0.05, xmax])
plt.xlabel(r"$am$", fontsize=18)
plt.ylabel(r"$M_{\eta_C}$ (GeV)", fontsize=18)
...
for ens in ens_list:
    bl_SMOM = bilinear_analysis(
        ens, mres=mres, loadpath=f"{folder}/{ens}_bl_massive_SMOM.p"
    )
    bl_mSMOM = bilinear_analysis(
        ens, mres=mres, loadpath=f"{folder}/{ens}_bl_massive_mSMOM.p"
    )
    for key in ["m", "mam_q"]:
        x, Zm_SMOM = bl_SMOM.massive_Z_plots(key=key, mu=mu_chosen, passinfo=True)
        Zm_SMOM, Zm_SMOM_err = Zm_SMOM.val[0], Zm_SMOM.err[0]
        x, Zm = bl_mSMOM.massive_Z_plots(key=key, mu=mu_chosen, passinfo=True)
        plot_dict[ens][key] = {
            "x_Z": x,
            "Zm": Zm.val,
            "Zm_err": Zm.err,
...
plt.axhspan(Z_SMOM - Z_SMOM_err, Z_SMOM + Z_SMOM_err, color="k", alpha=0.1)
...
fig, axes = plt.subplots(1, num_ens, sharey=True, figsize=(w * num_ens, h))
...
for ens in ["C1", "M1", "F1S"]:
    ...
    ax.errorbar(
        x, y, yerr=yerr, fmt="o", capsize=4, label="mSMOM", color=color_list[idx]
    )
    Z_SMOM = plot_dict[ens][key]["Zm_SMOM"]
    Z_SMOM_err = plot_dict[ens][key]["Zm_SMOM_err"]
    ax.axhspan(Z_SMOM - Z_SMOM_err, Z_SMOM + Z_SMOM_err, color="k", alpha=0.1)
...
fig, axes = plt.subplots(
    nrows=2, ncols=num_ens, sharex="col", sharey="row", figsize=(w * num_ens, h * 2)
)
plt.subplots_adjust(hspace=0, wspace=0)
for key in ["mam_q", "m"]:
...
for ens in ens_list:
    eta_star_dict = {float(eta): interpolate_eta_c(ens, eta) for eta in eta_stars}
...
for ens in ["C1", "M1", "F1S"]:
    ...
    axes[0, idx].errorbar(x, y, yerr=yerr, fmt="o", capsize=4, color=color_list[idx])
    ...
    y = plot_dict[ens][key]["Zm"]
    yerr = plot_dict[ens][key]["Zm_err"]
    axes[1, idx].errorbar(x, y, yerr=yerr, fmt="o", capsize=4, color=color_list[idx])
...
for ens in ["C1", "M1", "F1S"]:
    ...
    axes[0, idx].hlines(eta_C_star, xmin, xmax, color=color_list[3 + eta_C_star_idx])
    axes[0, idx].axvspan(
        am_star - am_star_err, am_star + am_star_err, color="k", alpha=0.1
    )
    axes[0, idx].vlines(
        am_star,
        ymin,
    ...
    axes[0, idx].text(
        am_star * 1.01, 0.3, r"$a_{" + ens + r"}m^\star$", rotation=270, fontsize=18
    )
...
for ens in ["M1", "C1", "F1S"]:
    ...
    am_stars_err = np.array([v[1] for k, v in plot_dict[ens]["eta_stars"].items()])
    xmin, xmax = axes_info[1, idx, 0, :]
    ...
    y_btsp = np.array([np.random.normal(y[i], yerr[i], N_boot) for i in range(len(y))])
    am_stars_btsp = np.array(
        [
            np.random.normal(v[0], v[1], N_boot)
    ...
    Zm_stars_err = np.array(
        [st_dev(Zm_stars_btsp[i, :], Zm_stars[i]) for i in range(len(Zm_stars))]
    )
    Zm_grain_err = np.array(
        [st_dev(Zm_grain_btsp[i, :], Zm_grain[i]) for i in range(len(Zm_grain))]
    )
...
for ens in ["M1", "C1", "F1S"]:
    ...
    yerr = plot_dict[ens][key]["Zm_err"]
    axes[1, idx].errorbar(
        x, y, yerr=yerr, fmt="o", capsize=4, color=color_list[idx], zorder=1
    )
    ...
    axes[1, idx].fill_between(
        am_grain, Zm_grain_up, Zm_grain_dn, color="k", alpha=0.3, label="fit", zorder=0
    )
...
for ens in ens_list:
    ...
    m_C_btsp = np.array(
        [ainv * am_C_btsp[:] * Zm_stars_btsp[i, :] for i in range(len(Zm_stars))]
    )
    m_C_err = np.array(
        [st_dev(m_C_btsp[i, :], m_C_ren[i]) for i in range(len(m_C_ren))]
    )
    ...
    am_stars_btsp = np.array(
        [
            np.random.normal(am_stars[i], am_stars_err[i], N_boot)
            for i in range(len(am_stars))
        ]
    )

    m_star_ren = ainv * am_stars * Zm_stars
    m_star_btsp = np.array(
        [ainv * am_stars_btsp[i, :] * Zm_stars_btsp[i, :] for i in range(len(Zm_stars))]
    )
    m_star_err = np.array(
        [st_dev(m_star_btsp[i, :], m_star_ren[i]) for i in range(len(m_star_ren))]
    )
...
for star in eta_stars:
    ...
    for key in ["m_C", "m_q"]:
        y = np.array([plot_dict[ens]["ren"][key + "_ren"][eta_idx] for ens in ens_list])
        yerr = np.array(
            [plot_dict[ens]["ren"][key + "_ren_err"][eta_idx] for ens in ens_list]
        )
        ...
        y_btsp = np.array(
            [plot_dict[ens]["ren"][key + "_ren_btsp"][eta_idx] for ens in ens_list]
        )
        ...
        m_grain_err = np.array(
            [st_dev(m_grain_btsp[i, :], m_grain[i]) for i in range(len(m_grain))]
        )
        m_grain_up = m_grain + m_grain_err
        m_grain_dn = m_grain - m_grain_err

        plot_dict["fits"][star].update(
            {
                key + "_cont": m_cont,
...
plt.plot(
    asq_grain,
    plot_dict["fits"][eta_star_chosen]["m_C_grain"],
    linestyle="dashed",
    color=color_list[3 + eta_C_star_idx],
    label=label,
)
plt.fill_between(
    asq_grain,
    plot_dict["fits"][eta_star_chosen]["m_C_grain_up"],
...
for ens in ens_list:
    ...
    plt.errorbar(x, y, yerr, fmt="o", capsize=4, color=color_list[idx])

plt.errorbar(
    [0],
    [plot_dict["fits"][eta_star_chosen]["m_C_cont"]],
...
for eta_C_star_idx in range(len(eta_stars)):
    ...
    label = (
        r"$M_{\eta_C}^\star=" + str(eta_star_chosen) + "$ GeV"
        if eta_star_chosen != eta_PDG.val
        else r"$M_{\eta_C}^{\star,PDG}$"
    )
    plt.plot(
        asq_grain,
        plot_dict["fits"][eta_star_chosen]["m_C_grain"],
...
for eta_C_star_idx in range(len(eta_stars)):
    ...
    plt.plot(
        asq_grain,
        plot_dict["fits"][eta_star_chosen]["m_q_grain"],
        linestyle="dashed",
        color=color_list[3 + eta_C_star_idx],
        label=label,
    )
    plt.fill_between(
        asq_grain,
        plot_dict["fits"][eta_star_chosen]["m_q_grain_up"],
    ...
    plt.errorbar(
        [0],
        [plot_dict["fits"][eta_star_chosen]["m_q_cont"]],
...
for eta_C_star_idx in range(len(eta_stars)):
    ...
    m_C_MS_btsp = np.array(
        [
            R_mSMOM_to_MSbar(mu_chosen, mbar_btsp[k]) * m_C_ren_btsp[k]
            for k in range(N_boot)
        ]
    ).reshape((200,))
    m_C_MS_err = st_dev(m_C_MS_btsp, m_C_MS)
    plot_dict["fits"][eta_star_chosen].update(
        {"m_C_MS": m_C_MS, "m_C_MS_err": m_C_MS_err, "m_C_MS_btsp": m_C_MS_btsp}
    )
