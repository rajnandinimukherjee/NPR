...
class bilinear_analysis:
    ...
    def __init__(self, ensemble, sea_mass_idx=0, scheme="gamma", cfgs=None, **kwargs):
        ...
        self.non_sea_masses = [
            "{:.4f}".format(info["masses"][k])
            for k in range(len(info["masses"]))
            if k != sea_mass_idx
        ]

        gauge_count = len(info["baseactions"])
        self.actions = [
            info["gauges"][i] + "_" + info["baseactions"][i] for i in range(gauge_count)
        ]
    ...
    def NPR(
        ...
        **kwargs,
        ...
        self.data = params[self.ens]['dataloc']+self.ens
        if self.ens[:3]=='KEK':
            self.data += '/results'
        else:
            if self.ens[-1] not in ["M", "S"] and self.ens[-1]!='0':
                self.data += "S/results"
            else:
                self.data += "/results"
        if not os.path.isdir(self.data):
            ...
            for b in tqdm(range(len(self.bl_list)), leave=False, desc=desc):
                ...
                prop1_info, prop2_info = decode_prop(prop1_name), decode_prop(
                    prop2_name
                )
                ...
                if condition1 and condition2:
                    ...
                    condition4 = (
                        prop1.momentum_squared == scheme * np.linalg.norm(mom_diff) ** 2
                    )
        ...
        for action in self.Z.keys():
            for masses in self.Z[action].keys():
                if len(self.Z[action][masses]) == 0:
                    ...
                    mom = m_grp.create_dataset("ap", data=self.momenta[action][masses])
                    for current in self.Z[action][masses][0].keys():
                        c_grp = m_grp.create_group(current)
                        Zs = stat(
                            val=[
                                self.Z[action][masses][mom][current].val
                                for mom in range(len(self.Z[action][masses]))
                            ],
                            err=[
                                self.Z[action][masses][mom][current].err
                                for mom in range(len(self.Z[action][masses]))
                            ],
                            btsp=np.array(
                                [
                                    self.Z[action][masses][mom][current].btsp
                                    for mom in range(len(self.Z[action][masses]))
                                ]
                            ).swapaxes(0, 1),
                        )
    ...
    def merge_mixed(self, **kwargs):
        ...
        for masses in list(mass_combinations):
            ...
            self.Z[(0, 1)][masses] = [
                {c: (res1[m][c] + res2[m][c]) / 2.0 for c in res1[m].keys()}
...
class fourquark_analysis:
        ...
        for action in self.Z.keys():
            for masses in self.Z[action].keys():
                if len(self.Z[action][masses]) == 0:
                    ...
                    Zs = stat(
                        val=[
                            self.Z[action][masses][mom].val
                            for mom in range(len(self.Z[action][masses]))
                        ],
                        err=[
                            self.Z[action][masses][mom].err
                            for mom in range(len(self.Z[action][masses]))
                        ],
                        btsp=np.array(
                            [
                                self.Z[action][masses][mom].btsp
                                for mom in range(len(self.Z[action][masses]))
                            ]
                        ).swapaxes(0, 1),
                    )
    ...
    def merge_mixed(self, **kwargs):
        ...
        for masses in list(mass_combinations):
            ...
            res2 = self.Z[(1, 0)][masses]
            self.Z[(0, 1)][masses] = [
                (res1[m] + res2[m]) / 2.0 for m in range(len(momenta))
            ]
