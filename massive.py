...
eta_PDG = stat(val=2983.9 / 1000, err=0.5 / 1000, btsp="fill")

mc_PDG = stat(val=1.2730, err=0.0046, btsp="fill")
...
class Z_bl_analysis:
    ...
    def __init__(self, ens, action=(0, 0), scheme="qslash", renorm="mSMOM", **kwargs):
        ...
        for m in self.data.keys():
            ...
            self.momenta[masses] = stat(
                val=self.data[m][self.grp]["ap"][:], btsp="fill"
            )
        ...
        df = pd.DataFrame(
            0,
            index=self.all_masses,
            columns=["amres", "aM", "Z_A", "Z_m(2.0)", "Z_m(2.5)", "Z_m(3.0)"],
        )
        df["amres"] = [
            err_disp(self.valence.amres[m].val, self.valence.amres[m].err)
            for m in self.all_masses
        ]
        df["aM"] = [
            err_disp(self.valence.eta_h_masses[m].val, self.valence.eta_h_masses[m].err)
            for m in self.all_masses
        ]
        df["Z_A"] = [
            err_disp(self.valence.Z_A[m].val, self.valence.Z_A[m].err)
            for m in self.all_masses
        ]
        for mu in [2.0, 2.5, 3.0]:
            df["Z_m(" + str(mu) + ")"] = [
                err_disp(
                    self.interpolate(mu, (m, m), "m").val,
    ...
    def plot_mass_dependence(
        ...
        **kwargs,
        ...
        x = join_stats(
            [self.valence.amres[m] + eval(m) for m in self.all_masses[start:stop]]
        )
        if key_only:
            ...
            if key == "m" and self.renorm == "SMOM":
                ...
                ax.errorbar(
                    ...
                    fmt="o",
                    c="k",
                    label="$Z_m^\mathrm{SMOM}:$" + err_disp(y0.val, y0.err),
                )
                ymin, ymax = ax.get_ylim()
                ax.vlines(x=0, ymin=ymin, ymax=ymax, color="k", linestyle="dashed")
                ...
                if normalise:
                    q = join_stats(
                        [
                            self.interpolate(mu, (m, m), "q")
                            for m in self.all_masses[start:stop]
                        ]
                    )
                    y = y / q
                    ax.set_ylabel(
                        r"$Z_" + key + "(\mu=" + str(mu) + r"\,\mathrm{GeV})/Z_q$"
                    )
            ...
            fig, ax = plt.subplots(nrows=nrows, ncols=1, sharex="col", figsize=figsize)
            plt.subplots_adjust(hspace=0, wspace=0)

            y = join_stats(
                [
                    self.valence.eta_h_masses[m] * self.ainv
                    for m in self.all_masses[start:stop]
                ]
            )
            ...
            am_c = stat(
                ...
                err="fill",
                btsp=[pred_amq(M.btsp[k], res.btsp[k]) for k in range(N_boot)],
            )
            am_star = stat(
                val=pred_amq(Mbar.val, res.val),
                err="fill",
                btsp=[pred_amq(Mbar.btsp[k], res.btsp[k]) for k in range(N_boot)],
            )
            ...
            if alt_fit:
                ...
                am_c = stat(
                    ...
                    err=(
                        max([var.err for var in variations]) ** 2
                        + np.abs(
                            (
                                max(var.val for var in variations)
                                - min(var.val for var in variations)
                            )
                            / 2
                        )
                        ** 2
            ...
            ax[0].errorbar(x.val, y.val, yerr=y.err, capsize=4, fmt="o")
            if add_pdg:
                ymin, ymax = ax[0].get_ylim()
                ax[0].hlines(y=M.val, xmin=0, xmax=am_c.val, color="k")
                ax[0].text(0.05, M.val - 0.05, r"$M$", va="top", ha="center", color="k")
                ax[0].fill_between(
                    np.linspace(0, am_c.val, 100),
                    M.val + M.err,
                    M.val - M.err,
                    color="k",
                    alpha=0.2,
                ...
                ax[0].vlines(
                    x=am_c.val, ymin=ymin, ymax=M.val, color="k", linestyle="dashed"
                )
                ax[0].text(
                    am_c.val,
                    0.5,
                ...
                ax[0].fill_between(
                    np.linspace(
                        am_star.val - am_star.err, am_star.val + am_star.err, 100
                    ),
                    ymin,
                    Mbar.val + Mbar.err,
                    color="r",
            ...
            y = join_stats(
                [self.interpolate(mu, (m, m), key) for m in self.all_masses[start:stop]]
            )
            if not Z_only:
                y = y * am_c * self.ainv
            ...
            res = fit_func(
                x, y, Z_m_ansatz, [0.1, 0.1, 0.1, 0.1], pause=False, verbose=False
            )
            Z_m_amstar = res.mapping(am_star)

            ax[1].errorbar(x.val, y.val, xerr=x.err, yerr=y.err, capsize=4, fmt="o")
            if add_pdg:
                ...
                ax[1].vlines(
                    x=am_star.val, ymin=ymin, ymax=ymax, color="r", linestyle="dashed"
                )
                ax[1].fill_between(
                    np.linspace(
                        am_star.val - am_star.err, am_star.val + am_star.err, 100
            ...
            Z_m_amc_SMOM = SMOM.plot_mass_dependence(
                mu,
                M,
                Mbar,
                key=key,
            ...
            label = (
                r"\,\mathrm{GeV})$"
                if Z_only
                else r"\,\mathrm{GeV})\cdot am_c\cdot a^{-1}\, [\mathrm{GeV}]$"
            )
            ...
            if alt_fit:
                ...
                Z_m_amstar = stat(
                    val=fit_vals[2], err=(stat_err**2 + sys_err**2) ** 0.5, btsp="fill"
                )
            ...
            if not Z_only:
                ...
                def Z_m_m_q_ansatz(am, param, **kwargs):
                    return (
                        param[0] * am + param[1] + param[2] * am**2 + param[3] * am**3
                    )
                ...
                res = fit_func(
                    x, y, Z_m_m_q_ansatz, [1, 1e-1, 1e-1, 1e-2], verbose=False
                )
                mbar = res.mapping(am_star)

                ax[2].errorbar(x.val, y.val, xerr=x.err, yerr=y.err, capsize=4, fmt="o")
                if add_pdg:
    ...
    def fit_variations(self, x, y, am_star, **kwargs):
        ...
        indices = np.sort(closest_n_points(am_star.val, x.val, n=2))
        res = fit_func(x[indices], y[indices], two_pt_ansatz, [1, 1e-1])
        fit_2 = res.mapping(am_star)

        def three_pt_ansatz(am, param, **kwargs):
            return param[0] + param[1] * am + param[2] * (am**2)

        indices = np.sort(closest_n_points(am_star.val, x.val, n=3))
        res = fit_func(x[indices], y[indices], three_pt_ansatz, [1, 1e-1, 1e-2])
        fit_3 = res.mapping(am_star)
        farthest_point = indices[-1]

        indices = list(np.sort(closest_n_points(am_star.val, x.val, n=4)))
        indices.remove(farthest_point)
        res = fit_func(x[indices], y[indices], three_pt_ansatz, [1, 1e-1, 1e-2])
        fit_3_alt = res.mapping(am_star)

        def four_pt_ansatz(am, param, **kwargs):
            return param[0] + param[1] * am + param[2] * (am**2) + param[3] * (am**3)

        indices = np.sort(closest_n_points(am_star.val, x.val, n=4))
        res = fit_func(x[indices], y[indices], four_pt_ansatz, [1, 1e-1, 1e-2, 1e-3])
        fit_4 = res.mapping(am_star)
        farthest_point = indices[-1]

        indices = list(np.sort(closest_n_points(am_star.val, x.val, n=5)))
        indices.remove(farthest_point)
        res = fit_func(x[indices], y[indices], four_pt_ansatz, [1, 1e-1, 1e-2, 1e-3])
    ...
    def interpolate(
        ...
        **kwargs,
        ...
        linear_Z = self.fit_momentum_dependence(
            mu, masses, key, fittype="linear", **kwargs
        )
        quadratic_Z = self.fit_momentum_dependence(
            mu, masses, key, fittype="quadratic", **kwargs
        )
        ...
        if plot:
            ...
            if pass_plot:
                ...
                ax.text(
                    0.5, 0.1, self.ens, va="center", ha="center", transform=ax.transAxes
                )
                ax.set_xlabel(r"$\sqrt{p^2}\,[\mathrm{GeV}]$")
                ax.set_ylabel(r"$Z_{" + key + r"}(a|p|, am_h=" + masses[0] + r")$")
                ymin, ymax = ax.get_ylim()
                ax.vlines(x=mu, ymin=ymin, ymax=ymax, color="k", linestyle="dashed")
                ax.set_ylim([ymin, ymax])
    ...
    def fit_momentum_dependence(
        ...
        **kwargs,
        ...
        if fittype == "linear":
            indices = np.sort(closest_n_points(mu, x.val, n=2))
            ...
            indices = np.sort(closest_n_points(mu, x.val, n=3))
            x_1, y_1 = x[indices[0]], y[indices[0]]
            x_2, y_2 = x[indices[1]], y[indices[1]]
    ...
    def plot_momentum_dependence(
        ...
        **kwargs,
        ...
        if stop != None:
            x, y = x[:stop], y[:stop]
        ax.errorbar(
            x.val, y.val, xerr=x.err, yerr=y.err, capsize=4, fmt="o", color="k", ms=4
        )
        xlabel = (
            r"$q^2\, \mathrm{[GeV]}^2$" if x_sq else r"$\sqrt{q^2}\,[\mathrm{GeV}]$"
        )
        ax.set_xlabel(xlabel)
        label = r"$am_\mathrm{input}=$" + masses[0]
        ax.text(
            0.5,
            1.0,
            label,
        ...
        ylabel = (
            r"$Z_" + key + r"/Z_q$" if normalise else r"$Z^\mathrm{mSMOM}_" + key + r"$"
        )
        ...
        if pass_fig:
            ...
            filename = (
                ...
                else f"plots/Z_{key}_v_ap.pdf"
            )
            call_PDF(filename)

    def plot_mass_momentum_dep(self, key, stop=None, mu=2.0, **kwargs):
        fig, ax = plt.subplots(figsize=(6, 4))
        N_mass = len(self.all_masses)
        # colors = [hls_to_rgb((2/3)*i/(N_mass-1),0.5,1) for i in range(N_mass)]

        # gg = Gradient(gradient_start='#0000ff', gradient_end='#ff0000', opacity=1.0)
        # input_list = np.linspace(0,1,N_mass)
        # colors = gg.get_gradient_series(series=input_list, fmt='html')
        colors = [plt.get_cmap("rainbow", N_mass)(i) for i in range(N_mass)]
        colors = color_list
        ...
        ax.vlines(
            mu, ymin=ymin, ymax=ymax * 1.05, color="k", linestyle="dashed", linewidth=1
        )
        ax_twin = ax.twiny()
        ax_twin.set_xlim(ax.get_xlim())
        ax_twin.set_xticks([mu])
        ax_twin.set_xticklabels([r"$\mu$"])
        ax.legend(ncol=3, frameon=False)
        xlabel = r"$\sqrt{q^2}\,[\mathrm{GeV}]$"
        ax.set_xlabel(xlabel)
        ylabel = r"$Z^\mathrm{mSMOM}_" + key + r"$"
        ax.set_ylabel(ylabel)

        filename = (
            kwargs["filename"] if "filename" in kwargs else f"plots/Z_{key}_v_ap_am.pdf"
        )
        call_PDF(filename)
    ...
    def save_data(self, filename="mNPR_data_for_tables.h5", **kwargs):
        ...
        for mu in [2.0, 2.5, 3.0]:
            Z = join_stats([self.interpolate(mu, (m, m), "m") for m in self.all_masses])
            ...
            Z = join_stats(
                [self.interpolate(mu, (m, m), "S") for m in self.all_masses[:4]]
            ) ** (-1)
            ...
            grp.create_dataset(
                f"SMOM/Z_m({str(mu)})/extrap/Bootstraps", data=SMOM_fit.btsp
            )

        file.close()
        print(f"Saved {self.ens} data to {filename}")

    def print_ens_Z_table(self, key="m", mus=[2.0, 2.5, 3.0], stop=None, **kwargs):
        if stop == None:
            stop = len(self.all_masses)
        am = join_stats(
            [self.valence.amres[m] + eval(m) for m in self.all_masses[:stop]]
        )
        N_mu = len(mus)
        ...
        for idx, am in enumerate(am):
            ...
            rv += [
                r"$"
                + "$ & $".join(
                    [err_disp(am.val, am.err)] + [err_disp(z.val, z.err) for z in Zs]
                )
                + r"$ \\"
            ]
        ...
        filename = (
            f"/Users/rajnandinimukherjee/PhD/thesis/inputs/MassiveNPR/"
...
class cont_extrap:
    def __init__(self, ens_list):
        ...
        self.mSMOM_dict = {ens: Z_bl_analysis(ens, renorm="mSMOM") for ens in ens_list}
        self.SMOM_dict = {ens: Z_bl_analysis(ens, renorm="SMOM") for ens in ens_list}

        self.a_sq = join_stats(
            [val.ainv ** (-2) for key, val in self.mSMOM_dict.items()]
        )
        self.extrap_values = {}
    ...
    def load_renorm_masses(self, mu, M, Mbar, **kwargs):
        ...
        for ens in self.ens_list:
            quantities = self.mSMOM_dict[ens].plot_mass_dependence(
                mu, M, Mbar, key="m", open_file=False, pass_vals=True, **kwargs
            )
        ...
        return join_stats(m_c_ren_mSMOM), join_stats(m_c_ren_SMOM), join_stats(m_bar)

    def plot_cont_extrap(
        self,
        mu,
        M,
        Mbar,
        ...
        open_file=True,
        verbose=False,
        mSMOM_color="tab:blue",
        SMOM_color="tab:orange",
        mSMOM_label="",
        amres_invert=False,
        phys_color=None,
        plot_details=False,
        **kwargs,
        ...
        if with_amres and len(self.ens_list) > 3:
            ...
            if quad:
                guess.append(1e-1)
                ansatz_name = (
                    r"$\alpha+\beta\,a^2+\gamma\,a^4+" + r"\delta\,am_\mathrm{res}(M)$"
                )
        else:
            ansatz_name = (
                r"$\alpha+\beta\,a^2+\gamma\,a^4$" if quad else r"$\alpha+\beta\,a^2$"
            )

        x = self.a_sq
        y_mc_mSMOM, y_mc_SMOM, y_mbar = self.load_renorm_masses(mu, M, Mbar)
        ...
        amres = join_stats(
            [self.mSMOM_dict[ens].valence.interpolate_amres(M) for ens in self.ens_list]
        )
        if with_amres:
            if plot_amres:
                ax.errorbar(
                    ...
                    color=mSMOM_color,
                    mfc="None",
                    alpha=1 if amres_invert else 0.3,
                )
        ...
        label1 += err_disp(M.val, M.err) if M_label == "" else M_label
        label2 += err_disp(Mbar.val, Mbar.err) if Mbar_label == "" else Mbar_label
        ...
        if plot_details and with_amres:
            h, l = ax.get_legend_handles_labels()
            white_patch = mpatches.Patch(color="white")
            if plot_SMOM:
                handles = [white_patch, h[0], h[1]] + [white_patch] * 12
                labels = ["", l[0], l[1]]
                labels += [
                    r"$m_R(a^2=0)$",
                    err_disp(res_mc_mSMOM.val[0], res_mc_mSMOM.err[0]),
                    err_disp(res_mc_SMOM.val[0], res_mc_SMOM.err[0]),
                ]
                labels += [
                    r"$C_\chi$",
                    err_disp(res_mc_mSMOM.val[2], res_mc_mSMOM.err[2]),
                    err_disp(res_mc_SMOM.val[2], res_mc_SMOM.err[2]),
                ]
                labels += [
                    r"$C_1$",
                    err_disp(res_mc_mSMOM.val[1], res_mc_mSMOM.err[1]),
                    err_disp(res_mc_SMOM.val[1], res_mc_SMOM.err[1]),
                ]
                labels += [
                    r"$p$-val",
                    "{:.2f}".format(res_mc_mSMOM.pvalue),
                    "{:.2f}".format(res_mc_SMOM.pvalue),
                ]
                ax.legend(
                    handles,
                    labels,
                ...
                handles = [white_patch, h[0]] + [white_patch] * 8
                labels = ["", l[0]]
                labels += [
                    r"$m_R(a^2=0)$",
                    err_disp(res_mc_mSMOM.val[0], res_mc_mSMOM.err[0]),
                ...
                labels += [r"$C_1$", err_disp(res_mc_mSMOM.val[1], res_mc_mSMOM.err[1])]
                labels += [r"$p$-val", "{:.2f}".format(res_mc_mSMOM.pvalue)]
                ax.legend(
                    handles,
                    labels,
                    ncols=5,
    ...
    def plot_M_variations(
        ...
        **kwargs,
        ...
        ax[0].text(
            -0.05, 1.0, r"[GeV]", va="bottom", ha="right", transform=ax[0].transAxes
        )
        ...
        for range_ in ranges:
            ...
            if verbose:
                print(
                    ...
                    + ": "
                    + " ".join(
                        [err_disp(res.val[i], res.err[i]) for i in range(len(guess))]
                    )
                )
            ...
            ax.text(0.5, 0.1, label, va="center", ha="center", transform=ax.transAxes)
            ax.errorbar(
                x.val[fitrange],
                y.val[fitrange],
                xerr=x.err[fitrange],
            ...
            ax.vlines(
                x=eta_PDG.val, ymin=ymin, ymax=ymax, color="k", linestyle="dashed"
            )
            ax.set_ylim([ymin, ymax])
            ax.text(
                0.5, 0.9, Mbar_label, va="center", ha="center", transform=ax.transAxes
            )
        ...
        sys_only = stat(val=mc_ren[0].val, err=sys_err, btsp="fill") * eta_PDG
        print(
            "mbar = "
            + err_disp(y_mbar_phys.val, y_mbar_phys.err)
            + ",\t m_c^R("
            + str(mu)
            + ") = "
            + err_disp(stat_only.val, stat_only.err, sys_err=sys_only.err)
        )
        mc_final = stat(
            val=mc_ren[0].val, err=(mc_ren[0].err ** 2 + sys_err**2) ** 0.5, btsp="fill"
        )
        ...
        if not pass_only:
            ...
            ax[0].vlines(x=1, ymin=ymin, ymax=ymax, color="k", linestyle="dashed")
            ...
            ax_twin.set_xticklabels([r"$M_{\eta_c}^\mathrm{PDG}$"], fontsize=8)
            ...
            ax[1].set_xticklabels(["[1.0,1.0]"] + range_names + ["final"], fontsize=8)
            ax[1].tick_params(axis="x", labelrotation=90)

            for fit_idx in range(n_fits - 1):
                label = r"$p$-val:" + "{:.2f}".format(pvalues[fit_idx])
                ax[1].text(
                    fit_idx + 1,
                    0.25,
                    label,
    ...
    def final_numbers(
        ...
        name_list=[r"0.6", r"0.7", r"0.75", r"0.8", r"0.9", r"1.0"],
        ...
        **kwargs,
        ...
        def get_mcmc(obj):
            return stat(
                ...
                err="fill",
                btsp=np.array(
                    [mcmc(obj.btsp[k], mu, f=4, N_f=4) for k in range(N_boot)]
                ),
            )
        ...
        if run:
            ...
            if data == "mSMOM":
                ...
                R_conv_mu = stat(
                    val=[R_mSMOM_to_MSbar(mu, mbar.val) for mbar in mbars],
                    err="fill",
                    btsp=np.array(
                        [
                            [R_mSMOM_to_MSbar(mu, mbar.btsp[k]) for mbar in mbars]
            ...
            mc_mus_MS.stat_only = join_stats(
                [mc_mus[i].stat_only * R_conv_mu[i] for i in range(N_Mbar)]
            )
            mc_mus_MS.sys_only = join_stats(
                [mc_mus[i].sys_only * R_conv_mu[i] for i in range(N_Mbar)]
            )
            ...
            mc_mus_MS.disp = [
                err_disp(
                    mc_mus_MS.val[i],
                    mc_mus_MS.stat_only.err[i],
                    sys_err=mc_mus_MS.sys_only.err[i],
                )
                + err_disp(mc_mus_MS.val[i], mc_mus_MS.PT_only.err[i], n=1)[-3:]
                for i in range(N_Mbar)
            ]
            ...
            mc_mus.disp = [
                err_disp(
                    mc_mus.val[i],
                    mc_mus.stat_only.err[i],
                    sys_err=mc_mus.sys_only.err[i],
                )
                for i in range(N_Mbar)
            ]

            mc_mcs_MS = join_stats([get_mcmc(mc_mus_MS[i]) for i in range(N_Mbar)])
            mc_mcs_MS.stat_only = join_stats(
                [get_mcmc(mc_mus_MS.stat_only[i]) for i in range(N_Mbar)]
            )
            mc_mcs_MS.sys_only = join_stats(
                [get_mcmc(mc_mus_MS.sys_only[i]) for i in range(N_Mbar)]
            )
            mc_mcs_MS.PT_only = join_stats(
                [get_mcmc(mc_mus_MS.PT_only[i]) for i in range(N_Mbar)]
            )
            mc_mcs_MS.disp = [
                err_disp(
                    mc_mcs_MS.val[i],
                    mc_mcs_MS.stat_only.err[i],
                    sys_err=mc_mcs_MS.sys_only.err[i],
                )
                + err_disp(mc_mcs_MS.val[i], mc_mcs_MS.PT_only.err[i], n=1)[-3:]
                for i in range(N_Mbar)
            ]

            if data == "mSMOM":
                self.extrap_values[mu] = {
                    "mbars": mbars,
                    "mSMOM_mc_mu": mc_mus,
        ...
        if plot:
            ...
            ax.errorbar(
                x.val, y.val, xerr=x.err, yerr=y.err, fmt="o", capsize=4, color="k"
            )
            ax.set_xlabel(r"$\overline{M}/M_{\eta_c}^\mathrm{PDG}$")
            ax.set_ylabel(
                r"$m_{c,R}^{\overline{\mathrm{MS}}}(\mu="
                + str(mu)
                + "\,\mathrm{GeV})\,[\mathrm{GeV}]$"
            )
        ...
        if verbose:
            print(
                f"mu = {mu}\nmbars: "
                + " ".join(self.extrap_values[mu]["mbars"].disp)
                + "\nm_c^mSMOM("
                + str(mu)
                + "): "
                + " ".join(self.extrap_values[mu]["mSMOM_mc_mu"].disp)
                + "\nm_c^MS<-mSMOM("
                + str(mu)
                + "): "
                + " ".join(self.extrap_values[mu]["mSMOM_mc_mu_MS"].disp)
                + "\nm_c^MS<-mSMOM(m_c): "
                + " ".join(self.extrap_values[mu]["mSMOM_mc_mc_MS"].disp)
                + "\nm_c^SMOM("
                + str(mu)
                + "): "
                + " ".join(self.extrap_values[mu]["SMOM_mc_mu"].disp)
                + "\nm_c^MS<-SMOM("
                + str(mu)
                + "): "
                + " ".join(self.extrap_values[mu]["SMOM_mc_mu_MS"].disp)
                + "\nm_c^MS<-SMOM(m_c): "
                + " ".join(self.extrap_values[mu]["SMOM_mc_mc_MS"].disp)
            )

    def RI_comparison_plot(self, mu=2.0, Mrat_list=[0.6, 0.7, 0.75], **kwargs):
        ...
        ax[1].errorbar(x, y.val, yerr=y.err, capsize=4, fmt="o", color=color_list[0])
        SMOM = self.extrap_values[mu]["SMOM_mc_mu_MS"]
        SMOM_stat = SMOM.stat_only
        SMOM_sys = SMOM.sys_only
        SMOM = stat(
            val=SMOM.val, err=(SMOM_stat.err**2 + SMOM_sys.err**2) ** 0.5, btsp="fill"
        )

        ax[0].errorbar(
            [0.0], SMOM.val, yerr=SMOM.err, fmt="d", capsize=4, color=color_list[1]
        )
        ...
        call_PDF("SMOM_v_mSMOM_mu.pdf")

    def final_plot(
        self, mu_list=[2.0, 2.5, 3.0], Mrat_list=[0.6, 0.7, 0.75], N_f=4, f=4, **kwargs
    ):
        ...
        for idx, mu in enumerate(mu_list):
            ...
            y.disp = [
                err_disp(y.val[i], y_stat.err[i], sys_err=y_sys.err[i])
                + err_disp(y.val[i], y_PT.err[i], n=1)[-3:]
            ...
            SMOM = stat(
                ...
                err=(SMOM_stat.err**2 + SMOM_sys.err**2) ** 0.5,
                btsp="fill",
            )
            SMOM.disp = [
                err_disp(SMOM.val[i], SMOM_stat.err[i], sys_err=SMOM_sys.err[i])
                for i in range(1)
            ]
    ...
    def mu_variations(self, mu_list, Mbar_list=[0.6, 0.7, 0.75], **kwargs):
        mc_mus = []
        for mu in mu_list:
            Mbar_list_eta = [eta_PDG * f for f in Mbar_list]
            mc_mus.append(
                self.final_numbers(
                    Mbar_list=Mbar_list_eta, mu=mu, plot=False, verbose=False
                )
            )

        x = stat(val=mu_list, err=np.zeros(len(mu_list)), btsp="fill")
        ...
        for Mbar_idx, Mbar in enumerate(Mbar_list):
            y = stat(
                ...
                btsp=np.array(
                    [[mcs.btsp[k, Mbar_idx] for mcs in mc_mus] for k in range(N_boot)]
                ),
            )
            label = r"$\overline{M}=" + str(Mbar) + r"\,M_{\eta_c}^\mathrm{PDG}$"
            ax.errorbar(
                x.val,
                y.val,
                yerr=y.err,
            ...
            ymin, ymax = ax.get_ylim()
            ax.vlines(x=mc_PDG.val, ymin=ymin, ymax=ymax, color="k", linestyle="dashed")
            ax.set_ylim([ymin, ymax])
    ...
    def plot_Mbar_variations(
        self, mu, M, Mbar_list, filename="", M_label="", name_list=[], **kwargs
    ):
        ...
        for Mbar_idx in tqdm(range(len(Mbar_list))):
            Mbar = Mbar_list[Mbar_idx]
            color = color_list[Mbar_idx]
            fit_dict = self.plot_cont_extrap(mu, M, Mbar, plot=False, **kwargs)
            end = fit_dict["end"]
            xrange = fit_dict["xrange"]
            if Mbar_idx == 0:
                xmax_store = xrange[-1]
        ...
        M_label = r"$M:" + err_disp(M.val, M.err) + r"$" if M_label == "" else M_label
    ...
    def CL_variations(
        self, mu, M, Mbar, plot=False, pass_axis=False, axis=None, **kwargs
    ):
        fit_dicts = []
        if plot:
            if pass_axis:
            ...
            kwargs.update(
                {"plot_SMOM": False, "pass_axis": True, "axis": axs, "mSMOM_color": "k"}
            )

        fit_dicts.append(
            self.plot_cont_extrap(
                mu,
                M,
                Mbar,
                with_amres=True,
                fit_alt=True,
                plot=plot,
                phys_color=color_list[0],
                mSMOM_label=r"$a^2+am_\mathrm{res}$ ",
                plot_amres=True,
                amres_invert=True,
                **kwargs,
            )
        ...
        fit_dicts.append(
            self.M_only.plot_cont_extrap(
                mu,
                M,
                Mbar,
                fit_alt=True,
                phys_color=color_list[1],
                plot=plot,
        ...
        if M.val < 0.81 * eta_PDG.val and Mbar.val < 0.81 * eta_PDG.val:
            fit_dicts.append(
                self.no_C1S.plot_cont_extrap(
                    mu,
                    M,
                    Mbar,
                    fit_alt=True,
                    phys_color=color_list[2],
                    plot=plot,
                    mSMOM_label=r"$a^2$ (no C1S) ",
                    plot_amres=False,
                    **kwargs,
                )
            )
...
def plot_quark_mass_coverage(
    ens_list=["F1S", "M1", "C1", "F1M", "M1M", "C1M"], **kwargs
):
    fig, ax = plt.subplots(figsize=(6, 4))
    # colors = [plt.get_cmap('rainbow', 3)(i) for i in range(3)]
    colors = color_list
    for ens_idx, ens in enumerate(ens_list):
        v = valence(ens)
        v.calc_all()
        x = join_stats([v.amres[m] + eval(m) for m in v.all_masses]) * v.ainv
        y = join_stats([v.eta_h_masses[m] * v.ainv for m in v.all_masses])
        ax.errorbar(
            x.val,
            y.val,
            xerr=x.err,
            ...
            linewidth=1,
            capsize=4,
            fmt="o" if ens[-1] == "M" else "s",
            c=colors[ens_idx % 3],
            label=ens + "S" if ens[-1] == "1" else ens,
        )

    xmin, xmax = ax.get_xlim()
    ax.hlines(
        eta_PDG.val, xmin=0, xmax=xmax, color="k", linestyle="dashed", linewidth=1
    )
    ax.set_xlim(0, xmax)
    ymin, ymax = ax.get_ylim()
    ax.set_ylim(0, ymax)
    ...
    ax_twin.set_yticks([eta_PDG.val])
    ax_twin.set_yticklabels([r"$M_{\eta_c}^\mathrm{PDG}$"])

    xlabel = r"$m_q = (am_\mathrm{input}+am_\mathrm{res})\cdot a^{-1}\,[\mathrm{GeV}]$"
    ax.set_xlabel(xlabel)
    ylabel = r"$M_{\eta_h}\,[\mathrm{GeV}]$"
    ax.set_ylabel(ylabel)
    filename = kwargs["filename"] if "filename" in kwargs else f"plots/M_eta_v_am.pdf"
    call_PDF(filename)
