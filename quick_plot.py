...
for i, j in itertools.product(range(len(operators)), range(len(operators))):
    if mask[i, j]:
        ...
        for ens in ens_list:
            ...
            ax[0].errorbar(x, y, yerr=yerr, xerr=xerr, fmt="o:", capsize=4, label=ens)
            ...
            ax[1].errorbar(x, y, yerr=yerr, xerr=xerr, fmt="o:", capsize=4, label=ens)
        ax[0].set_ylabel(
            r"$Z_{" + str(i + 1) + str(j + 1) + "}^{" + ",".join(ens_list) + r"}/Z_A^2$"
        )
