...
Z_dict = {
    masses[i]: Z_analysis(ens, norm="bag", sea_mass_idx=i) for i in range(num_masses)
}
...
for mom in momenta:
    ...
    for i, j in itertools.product(range(5), range(5)):
        if fq_mask[i, j]:
            y = join_stats(
                [
                    stat(
                        ...
                        btsp=Z_dict[m].Z.btsp[
                            :, list(Z_dict[m].am.val).index(mom), i, j
                        ],
                    )
                    for m in masses
                ]
            ...
            ax[i, j].errorbar(x, y.val, yerr=y.err, fmt="o", capsize=4, label="C1M")
            ax[i, j].errorbar(
                [float(comp_Z.sea_m)],
                y_comp.val,
                yerr=y_comp.err,
