...
class ens_table:
    ...
    def __init__(
        self, ens, norm="V", sea_mass_idx=0, action=(0, 0), scheme="gamma", **kwargs
    ):
        self.ens = ens
        self.action = action
        self.scheme = scheme
        self.norm = norm
        self.Z_obj = Z_analysis(
            self.ens,
            norm=norm,
            action=action,
        ...
        a1, a2 = self.action
        datafile = (
            f"/Users/rajnandinimukherjee/PhD/NPR/data/" + f"action{a1}_action{a2}/"
        )
        datafile += "__".join(
            [
                "NPR",
    ...
    def create_Z_table(self, indices=None, filename=None, **kwargs):
        if "rotate" in kwargs:
            ...
            self.Z = stat(
                val=[
                    rot_mtx @ self.Z.val[m] @ np.linalg.inv(rot_mtx)
                    for m in range(self.N_mom)
                ],
                err="fill",
                btsp=[
                    [
                        rot_mtx @ self.Z.btsp[k, m] @ np.linalg.inv(rot_mtx)
                        for m in range(self.N_mom)
                    ]
        ...
        if indices == None:
            for row in range(2):
                ...
                rv += [
                    r"$a\mu$ & $"
                    + "$ & $".join(
                        [str(np.around(m, 5)) for m in list(self.am[start:end])]
                    )
                    + r"$ \\"
                ]
                rv += [r"\hline"]
                for i, j in itertools.product(range(self.N_ops), range(self.N_ops)):
                    if self.Z_obj.mask[i, j]:
                        Z_disp = [
                            err_disp(self.Z.val[m, i, j], self.Z.err[m, i, j])
                ...
                rv += [r"\hline"]
                ratio_disp = [
                    err_disp(self.ratio.val[m], self.ratio.err[m])
                    for m in range(start, end)
                ]
                rv += [r"$Z_A/Z_S$ & $" + r"$ & $".join(ratio_disp) + r"$ \\"]
                rv += [r"\hline"]
        else:
            momenta = r" & ".join([str(np.around(self.am[m], 5)) for m in indices])
            rv += [r"$a\mu$ & " + momenta + r" \\"]
            rv += [r"\hline"]
            for i, j in itertools.product(range(self.N_ops), range(self.N_ops)):
                if self.Z_obj.mask[i, j]:
                    ...
                    if i == j:
                        if i == 0 or i == 2 or i == 4:
                            rv += [r"\hline"]
            ratio_disp = [
                err_disp(self.ratio.val[m], self.ratio.err[m]) for m in indices
            ]
            rv += [r"$Z_A/Z_S$ & $" + r"$ & $".join(ratio_disp) + r"$ \\"]
            rv += [r"\hline"]
...
class extrap_table:
    ...
    def create_Z_table(self, mu, filename=None, **kwargs):
        ...
        ainvs = r" & ".join(
            [
                err_disp(params[ens]["ainv"], params[ens]["ainv_err"])
            ...
            if self.Z_fit.mask[i, j]:
                Zs = r" & ".join(
                    [
                        err_disp(
                            Z_assign[e].val[i, j],
                            Z_assign[e].stat_err[i, j],
                            n=2,
                            sys_err=Z_assign[e].sys_err[i, j],
                        )
    ...
    def create_sig_table(self, mu1, mu2, filename=None, **kwargs):
        ...
        for i, j in itertools.product(range(5), range(5)):
            if self.Z_fit.mask[i, j]:
                s_vals = r" & ".join(
                    [
                        err_disp(sigmas[e].val[i, j], sigmas[e].err[i, j])
                        for e in self.sig.relevant_ensembles
                    ]
                )
    ...
    def print_sigmas(self, mu1, mu2, stepsize=1.0, **kwargs):
        ...
        for m in range(1, len(mus)):
            sig = (
                self.sig.calc_running(mus[m - 1], mus[m], chi_sq_rescale=True, **kwargs)
                @ sig
            )
        ...
        for j in range(5):
            rv += [
                " & ".join([err_disp(sig.val[i, j], sig.err[i, j]) for i in range(5)])
                + r" \\"
            ]

        rv += [r"\end{bmatrix}"]
        filename = (
            f"/Users/rajnandinimukherjee/Desktop/draft_plots/"
